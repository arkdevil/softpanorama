...
inline        streambuf(char* p, int l) { setbuf(p,l); alloc = 0; }
...
inline        filebuf() { opened = 0; fp = 0; }
inline        filebuf(FILE* p) { fp = p; opened = 1; }
inline        filebuf(int nfd) { fd = nfd; opened = 1; }
inline        filebuf(int nfd, char* p, int l) : (p,l) { fd = nfd; opened = 1; }
inline        ~filebuf() { close(); }
};
...
inline                ostream(streambuf* s) { bp = s; state = 0; }
inline                ostream(int fd) { bp = new filebuf(fd); state = 0; }
inline                ostream(int size, char* p)
                {
                        state = 0;
                        bp = new streambuf();
...
inline                istream(streambuf* s, int sk =1, ostream* t =0) // bind to buffer
                {
                        state = 0;
                        skipws = sk;
                        tied_to = t;
...
inline                istream(int size, char* p, int sk =1)           // bind to string
                {
                        state = 0;
                        skipws = sk;
                        tied_to = 0;
...
inline                istream(int fd, int sk =1, ostream* t =0)       // bind to file
                {
                        state = 0;
                        skipws = sk;
                        tied_to = t;
