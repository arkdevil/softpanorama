...
/* $Log: co.c%v $
 * Revision 1.2  1991/08/20  07:14:15  SGP
...
mainProg(coId, "co", "$Id: co.c%v 1.2 1991/08/20 07:14:15 SGP Exp $")
{
 ...
 */
{
	...
	static int
rmlock(const struct hshentry *delta)
 ...
 */
{
	register const char *j;
		...
		if (!(j = addjoin((char *)j))) return false;
				while ((*j==' ') || (*j=='\t')) j++;
				if (*j == ':') {
						j++;
						while((*j==' ') || (*j=='\t')) j++;
	...
	static const char *
getancestor(const char *r1, const char *r2)
 ...
 */
{
	...
	static int
buildjoin(const char *initialfile)
