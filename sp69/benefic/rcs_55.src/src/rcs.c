...
/* $Log: rcs.c%v $
 * Revision 1.2  1991/08/23  13:32:15  SGP
...
mainProg(rcsId, "rcs", "$Id: rcs.c%v 1.2 1991/08/23 13:32:15 SGP Exp $")
{
...

{
	...
	static void
getchaccess(const char *login, enum changeaccess command)
{
...
rmnewlocklst(const struct Lockrev *which)
...
updateaccess()
{
	...
	static int
sendmail(const char *Delta, const char *who)
 ...
 */
{
	...
	static void
breaklock(const struct hshentry *delta)
...
searchcutpt(const char *object, unsigned length, struct hshentries *store)
 ...
 */
{
	...
	static void
rcs_setstate(const char *rev,const char *status)
 ...
 */
{
	...
	static int
buildeltatext(const struct hshentries *deltas)
