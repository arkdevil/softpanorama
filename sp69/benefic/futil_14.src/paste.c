/* paste - merge lines of files
   ...
   Usage: paste [-s] [-d delim-list] [+serial] [+delimiters delim-list]
          [file...]
...
#ifdef MSDOS
static char RCS_Id[] =
  "$Header: e:/gnu/fileutil/RCS/paste.c 1.4.0.3 90/09/19 12:11:19 tho Exp $";

static char Program_Id[] = "paste";
...
char *collapse_escapes (char *strptr);
int paste_parallel (int nfiles, char **fnamptr);
int paste_serial (int nfiles, char **fnamptr);
void usage (void);
     ...
     char **argv;
{
  ...
  while ((optc = getopt_long (argc, argv, "d:s", longopts, &longind))
	 != EOF)
    {
      switch (optc)
	{
  ...
  if (!serial_merge)
    exit (paste_parallel (argc - optind, &argv[optind]));
  else
    exit (paste_serial (argc - optind, &argv[optind]));
}
     ...
     char **fnamptr;
{
  ...
  for (; nfiles; nfiles--, fnamptr++)
      ...
      if (charold == EOF)
	{
	  ...
	  if (ferror (fileptr))
	    {
	      error (0, errno, "error reading %s", *fnamptr);
	      errors = 1;
	    }
	  putc ('\n', stdout);
