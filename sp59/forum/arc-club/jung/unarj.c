 ...
 * 28/06/93  Груздев А.  Добавлена работа с паролем
 *
 */
...
"        UNARJ [-gpass] e archive     (extract archive)\n",
"        UNARJ [-gpass] t archive     (test archive)\n",
"        UNARJ [-gpass] x archive     (extract with pathnames)\n",
"\n",
...
char M_VERSION [] = "UNARJ (Demo version) 2.30 Copyright (c) 1991 Robert K Jung\n"
                    "(Опцию работы с паролем добавил Груздев Александр, г.Пермь)\n\n";
char M_ARCDATE [] = "Archive date      : %s\n";
char M_BADSWTCH[] = "Bad UNARJ switch: %s";      /**/
...
static uchar  password [50];  /**/
static uchar *origpass=0;     /**/  
static int    glength=0;      /**/
static int    gletter=0;      /**/
static uchar  garb_char;      /**/ 
...
char *name;
{
    ...
    method = get_byte();
    file_type = get_byte();
    garb_char = get_byte();       /* (void)get_byte(); */
...
check_flags()
{
    ...
    if ((arj_flags & GARBLE_FLAG) != 0)
    {
        ...
        {                                                /**/
           ...
           gletter = 0;                                  /**/
           for (i=0; i<glength; i++)                     /**/
               password [i] = origpass [i] + garb_char;  /**/
        }                                                /**/
    }
...
char *argv[];
{
    ...
    {                                         /**/
        if (strlen(argv[1]) < 3 ||            /**/
            argv [1][0] != '-'  ||            /**/
            toupper(argv [1][1]) != 'G')      /**/
            error(M_BADSWTCH, argv[1]);       /**/
       origpass = argv [1] + 2;               /**/
       glength  = strlen (origpass);          /**/
       if (strlen(argv[2]) > 1)               /**/
          error(M_BADCOMND, argv[2]);         /**/
       command = toupper(*argv[2]);           /**/
       if (strchr("ETX", command) == NULL)    /**/
          error(M_BADCOMND, argv[2]);         /**/
       arc_p = argv[3];                       /**/
    }                                         /**/
    else
