	...
	do sn=1,linnum
	  ...
	  do cn=1,cnmax
	    call retoch(val(cn+(sn-1)*colnum),buf)
	    xx=x-8+13*cn
 ...
 2	j=1
	...
	sym=char(inkey)
	if(sym.ge.' ') then
	  if(sym.eq.' '.or.sym.eq.'E'.or.sym.eq.'e'
     *	    .or.(sym.ge.'0'.and.sym.le.'9').or.sym.eq.'+'
     *	    .or.sym.eq.'-'.or.sym.eq.'.') then
	    ...
	    buf(48)=sym
	    bp=bp+1
	    call wrtext(buf(37),12,xx,yy,12,sa2)
	  else
	    call beep
	  endif
	...
	elseif(inkey.eq.13) then
	  if(bp.gt.0) then
	    call chtore(buf,val((sn-1)*colnum+cn))
	    call info(sn,cn)
	    bp=0
	  endif
	  call retoch(val((sn-1)*colnum+cn),buf)
	...
	elseif(inkey.eq.27) then
	  ...
	  elseif(scan.eq.72.or.scan.eq.73) then
	    ...
	    do i=1,j
		  ...
		  cnold=cn
		  do cn=1,cnmax
		    call retoch(val((sn-1)*colnum+cn+nsc),buf)
		    xx=x-8+13*cn
	  ...
	  elseif(scan.eq.75) then
	    ...
	    elseif(cn.gt.1) then
	      ...
	      do sn=1,dy
	        yy=y+sn-1
		call retoch(val((sn+nss-1)*colnum+cn),buf)
		call wrtext(buf,12,xx,yy,12,0)
	      enddo
	      sn=snold
	      yy=yyold
