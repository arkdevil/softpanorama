// SPLIRMRB - splits MRB into SHG/BMP/WMF and SHG into BMP/WMF files - Version 1.3
...
//        
// Produced files will be created in the current directory using extensions *.EGA,
// *.VGA, *.CGA, *.854 or *.Snn, *.nnn where n is a digit from 0 to 9.
// Discarded hotspot info will be written to stdout.
// 
...
long DecodeData(FILE *f,unsigned long dwDataSize,FILE *fTarget,unsigned long dwRawSize,unsigned char byPacked)
{
    unsigned long dwTargetSize;
    long start;
    ...
    case 1: // runlen
        while(dwDataSize>0)
        {
            ...
            if(count&0x80)
            {
                count&=0x7F;
                dwDataSize-=count;
                copy(f,count,fTarget);
            }
            else
            {
                dwDataSize--;
                value=getc(f);
                while(count-->0) putc(value,fTarget);
            }
        }
        break;
    default:
        fprintf(stderr,"unknown packing method: %02X.\n",byPacked);
        return 0L;
    }
    dwTargetSize=ftell(fTarget)-start;
    if(dwTargetSize<dwRawSize)
    {
        fprintf(stderr,"Picture decompression produced %lu bytes instead of required %lu\n",dwTargetSize,dwRawSize);
    }
    else if(dwTargetSize>dwRawSize)
    {
        fprintf(stderr,"Picture decompression produced %ld bytes more than required.\n",dwTargetSize-dwRawSize);
    }
    return dwTargetSize;
}
...
int main(int argc,char *argv[])
{
    ...
    long dwDataSize,dwHotspotOffset,dwHotspotSize,dwRawSize,w,h,offset;
    ...
    if(argc<2)
    {
        fprintf(stderr,"SPLITMRB - splits MRB into SHG/BMP/WMF and SHG into BMP/WMF files - Version 1.3\n"
               "M.Winterhoff, Geschw.-Scholl-Ring 17, 38444 Wolfsburg, Germany, CIS 100326,2776\n\n"
               "usage: SPLITMRB filename[.mrb] ...\n"
               "       SPLITMRB filename.shg ...\n\n"
               "Produced files will be created in the current directory using extensions *.EGA,\n"
            ...
            else
            {
                ...
                for(j=0;j<n;j++)
                {
                    ...
                    if(byType==6||byType==5&&byPacked<2)
                    {
                        ...
                        /* bmih.biXPelsPerMeter = 40L* */ w=GetCDWord(f);
                        /* bmih.biYPelsPerMeter = 40L* */ h=GetCDWord(f);
                        ...
                        else
                        {
                            ...
                            if(fTarget)
                            {
                                ...
                                {
                                    DecodeData(f,dwDataSize,fTarget,bmih.biSizeImage,byPacked);
                                }
                    ...
                    else if(byType==8) // Windows MetaFile
                    {
                        ...
                        else
                        {
                            ...
                            if(fTarget)
                            {
                                ...
                                DecodeData(f,dwDataSize,fTarget,dwRawSize,byPacked);
                                fclose(fTarget);
                            }
                            if(dwHotspotOffset)
                            {
