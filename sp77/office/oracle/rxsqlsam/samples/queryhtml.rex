...
/***********************************************************************/
/* Program     : queryhtml.rex                                         */
/* Version     : 1.00                                                  */
/* Author      : Mark Hessling                                         */
/* Date        : 13 Aug 1995                                           */
/* Purpose     : This program runs as a CGI-BIN executable and provides*/
/*             : a user with a WWW browser the ability to execute      */
/*             : queries on a SQL database.  The one program accepts   */
/*             : database/user and the query the user wants to run.    */
/*             : This program also displays the formatted result of the*/
/*             : query.                                                */
/* Arguments   : None.                                                 */
...
Say '<H2>Results of query:</H2><P>'
Say '<H3>'query'</H3><P>'
If sqlconnect('TEST',arg1,arg2) < 0 Then Abort()
If Translate(Substr(query,1,6)) \= 'SELECT' Then
   Do
     Say "<H2>SELECT commands only please; we wouldn't want to muck up the data now would we?</H2>"
     Return
   End
...
/***********************************************************************/
formstart: Procedure
Say '<FORM METHOD="POST" ACTION="http://boojum3/cgi-bin/queryhtml.rex">'
Return
