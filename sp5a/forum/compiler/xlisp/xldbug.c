...
{
    /* check for a fatal or recursive error */
    if (s_breakenable->n_symvalue == NULL)
	xlsignal(emsg,arg);
...
xldinit()
{
    if ((trace_stack = (NODE **) malloc(sizeof(NODE *) * TDEPTH)) == NULL)
	xlabort("insufficient memory");
    total += (long) (sizeof(NODE *) * TDEPTH);
    xltrace = -1;
    xldebug = 0;
}
