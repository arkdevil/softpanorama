  ...
  NODE *args;
{
    ...
    for (val = NULL; consp(list.n_ptr); list.n_ptr = cdr(list.n_ptr))
  ...
  NODE *args;
{
    ...
    /* look for the expression */
    for (val = NULL; consp(alist.n_ptr); alist.n_ptr = cdr(alist.n_ptr))
  ...
  NODE *args;
{
    ...
    arg = xlarg(&args);
    xllastarg(args);
    return (arg == NULL || symbolp(arg) ? true : NULL);
}
  ...
  NODE *args; int (*fcn)();
{
    ...
    return ((*fcn)(arg1,arg2) ? true : NULL);
}
