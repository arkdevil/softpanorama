		 ...
		 { /* Set to 5 if only one repetition is allowed and
	      ...
	      /* If an invalid interval, match the characters as literals.  */
	       if (beg_interval)
                 ...
                 assert ("No interval beginning to which to backtrack" == 0);
...
/* Using the compiled pattern in PBUFP->buffer, first tries to match the
   ...
   - 1, etc.  It is up to the caller to make sure that RANGE is not so
   large as to take the starting position outside of the input strings.
   STRING1 and STRING2 are of SIZE1 and SIZE2, respectively.  In REGS,
   return the indices of the virtual concatenation of STRING1 and
   STRING2 that matched the entire PBUFP->buffer and its contained
   subexpressions.  Do not consider matching past the index MSTOP in the
...
#define PREFETCH							\
 while (d == dend)						    	\
  {									\
    if (dend == end_match_2) goto fail;  /* end of string2 => fail.  */	\
...
/* Match the pattern described by PBUFP against the virtual
   ...
   consider matching past the index MSTOP in the virtual concatenation
  ...
  while (1)
	  ...
	  break;
        ...
        case succeed_n: 
	      ...
	      p[2] = unused;
            ...
            assert ("The succeed_n's n is not set" == 0);
          break;
	...
	case wordbeg:
	  if (d == end2  /* Points to end?  */
	    ...
	    goto fail;
	  if (d == string1  /* Points to first char?  */
	      || SYNTAX (d[-1]) != Sword)  /* Previous char not a letter?  */
	    break;
	  goto fail;
