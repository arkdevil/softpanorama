int Config()
{
  struct InitDialogItem InitItems[]={
    ...
    DI_CHECKBOX,5,5,0,0,0,0,0,0,(char *)MConfigSkipTopLevel,
  ...
  DialogItems[5].Selected=Opt.SkipTopLevel;
  int ExitCode=Info.Dialog(Info.ModuleNumber,-1,-1,76,10,"NetCfg",DialogItems,sizeof(DialogItems)/sizeof(DialogItems[0]));
  if (ExitCode!=7)
    return(FALSE);
  Opt.AddToDisksMenu=DialogItems[1].Selected;
  Opt.DisksMenuDigit=atoi(DialogItems[2].Data);
  Opt.SkipTopLevel=DialogItems[5].Selected;
  ...
  SetRegKey(HKEY_CURRENT_USER,"","SkipTopLevel",Opt.SkipTopLevel);
  return(TRUE);
}
