...
int PluginClass::DeleteFiles(struct PluginPanelItem *PanelItem,int ItemsNumber,int OpMode)
{
  ...
  ArcCommand ArcCmd(PanelItem,ItemsNumber,Command,ArcName,CurDir,"",AllFilesMask,IgnoreErrors);
...
int PluginClass::ProcessHostFile(struct PluginPanelItem *PanelItem,int ItemsNumber,int OpMode)
{
  ...
  *Password=0;
  ...
  ArcCommand ArcCmd(PanelItem,ItemsNumber,Command,ArcName,"",Password,AllFilesMask,
                    IgnoreErrors,CommandType==CMD_COMMENT || CommandType==CMD_COMMENTFILES ? 2:0);
                          ...
                          int Move,char *DestPath,int OpMode)
{
  ...
  if ((OpMode & (OPM_FIND|OPM_VIEW|OPM_EDIT))==0 && CurArcInfo.Volume && *CurDir==0)
  {
    char VolMsg[300];
    ...
    if (MsgCode==1)
    {
      ...
      if (ItemsInfo.Encrypted)
        MaskPanelItem.FindData.dwFileAttributes=F_ENCRYPTED;
  ...
  ArcCommand ArcCmd(PanelItem,ItemsNumber,Command,ArcName,CurDir,
             ...
             (OpMode & OPM_VIEW)!=0,(OpMode & OPM_FIND));
                          ...
                          int Move,int OpMode)
{
  ...
  *Password1=*Password2=0;
  ...
  while (1)
  {
    const char *HistoryName="ArcName";
    struct InitDialogItem InitItems[]={
    ...
    if ((OpMode & OPM_SILENT)==0)
    {
      ...
      {
        char *NamePtr;
        GetFullPathName(DialogItems[2].Data,sizeof(ArcName),ArcName,&NamePtr);
      }
    }
    ...
    ArcCommand ArcCmd(PanelItem,ItemsNumber,Command,DialogItems[2].Data,"",
                      Password1,AllFilesMask,IgnoreErrors);
    if (!IgnoreErrors && ArcCmd.GetExecCode()!=0)
      ArcExitCode=0;
    break;
  }
...
int PluginClass::ProcessKey(int Key,unsigned int ControlState)
{
  if ((ControlState & PKF_ALT) && Key==VK_F6)
  {
    ...
    Info.Control(this,FCTL_REDRAWANOTHERPANEL,(void *)1);
    return(TRUE);
  }
  return(FALSE);
}
