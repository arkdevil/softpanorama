                               ...
                               int Silent)
{
  ...
  if (*Command)
  {
    ...
    if (Hide==1 && CommandType==0 || CommandType==2)
      Hide=0;
    ExecCode=Execute(Command,Hide,Silent,!*Password);
    if (!IgnoreErrors && ExecCode!=0)
    {
      char ErrMsg[200];
      sprintf(ErrMsg,GetMsg(MArcNonZero),ExecCode);
      char *MsgItems[]={GetMsg(MError),ErrMsg,GetMsg(MOk)};
...
int ArcCommand::ReplaceVar(char *Command,int &Length)
{
  ...
  int QuoteName=FALSE,UseSlash=FALSE,FolderMask=FALSE,FolderName=FALSE;
  int NameOnly=FALSE;
  ...
  switch(Command[2])
  {
    ...
    case 'L':
      if (!MakeListFile(ListFileName,Command[2]=='l',QuoteName,UseSlash,
                        FolderName,NameOnly,FolderMask,LocalAllFilesMask))
        return(-1);
      char QListName[NM+2];
...
int ArcCommand::MakeListFile(char *ListFileName,int ShortNames,int QuoteNames,
                int UseSlash,int FolderName,int NameOnly,int FolderMask,
                char *LocalAllFilesMask)
{
  FILE *ListFile;
