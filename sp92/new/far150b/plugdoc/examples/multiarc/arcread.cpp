...
int PluginClass::ReadArchive(char *Name)
{
  ...
  while (1)
  {
    ...
    *CurArcData.FindData.cAlternateFileName=0;
    ...
    PluginPanelItem *NewArcData=(PluginPanelItem *)realloc(ArcData,(ArcDataCount+1)*sizeof(*ArcData));
    if (NewArcData==NULL)
      break;
  ...
  switch(GetItemCode)
  {
    case GETARC_BROKEN:
      {
        char *MsgItems[]={GetMsg(MError),GetMsg(MBadArchive),GetMsg(MOk)};
        Info.Message(Info.ModuleNumber,FMSG_WARNING,NULL,MsgItems,sizeof(MsgItems)/sizeof(MsgItems[0]),1);
      }
      break;
    case GETARC_UNEXPEOF:
      {
        char *MsgItems[]={GetMsg(MError),GetMsg(MUnexpEOF),GetMsg(MOk)};
        Info.Message(Info.ModuleNumber,FMSG_WARNING,NULL,MsgItems,sizeof(MsgItems)/sizeof(MsgItems[0]),1);
      }
      break;
    case GETARC_READERROR:
      {
        char *MsgItems[]={GetMsg(MError),GetMsg(MReadError),GetMsg(MOk)};
        Info.Message(Info.ModuleNumber,FMSG_WARNING,NULL,MsgItems,sizeof(MsgItems)/sizeof(MsgItems[0]),1);
      }
      break;
  }
...
int PluginClass::GetFindData(PluginPanelItem **pPanelItem,int *pItemsNumber,int OpMode)
{
  ...
  if (CompareFileTime(&NewArcFindData.ftLastWriteTime,&ArcFindData.ftLastWriteTime)!=0 ||
      NewArcFindData.nFileSizeLow!=ArcFindData.nFileSizeLow)
    if (!ReadArchive(ArcName))
      return(FALSE);
  ...
  *pItemsNumber=0;
  for (int I=0;I<ArcDataCount;I++)
  {
    ...
    if (strlen(Name)>CurDirLength && strnicmp(Name,CurDir,CurDirLength)==0 &&
