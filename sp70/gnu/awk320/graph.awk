...
$1 == "bottom" && $2 == "ticks" {       # ticks for y-axis
    for (i = 3; i <= NF; i++) bticks[++nb] = $i
    next
}
$1 == "left" && $2 == "ticks" {         # ticks for x-axis
...
END {       # draw graph
    if (xmin == "") {           # no range was given
        ...
        ymin = ymax = y[1]
        for (i = 2; i <= nd; i++) {
            if (x[1] < xmin) xmin = x[i]
            if (x[1] > xmax) xmax = x[i]
            if (y[1] < ymin) ymin = y[i]
            if (y[1] > ymax) ymax = y[i]
        }
    }
...
function ticks(     i) {    # creadt tick marks for both axes
    for (i = 1; i <= nb; i++) {
