...
{  Я  нарушил  авторские  права  фирмы  Borland,  но  уверен,  что кое-кому
  изменения   и  дополнения,  внесенные  в  этот  модуль  облегчат  жизнь и
  сэкономят  время,  да  и  сами  буржуйские  программисты могут почерпнуть
  отсюда  немало  полезного для себя. Поэтому я и решил выпустить этот опус
  "в   люди".   Изменения   вносились   и   вносятся   постепенно  по  мере
  потребности   в  них.  Все  дополнения  опробовались  и  особых  "глюков"
  замечено  не  было.  Если же они выскочат, то сообщите, пожалуйста, мне о
  них.  Пояснения  к  изменениям  (документацией  я это назвать не могу) Вы
  можете найти отдельно в файле objects.doc. С уважением, автор.
    Пишите на NTS BBS ((095)231-4682; FidoNet 2:5020/224)
    или звоните на NETACS BBS ((095)913-6235, 913-6239; FidoNet нет) }
...
{ TPrinterStream }
  ...
  TPrinterStream = object(TDosStream)
    constructor Init;
    destructor  Done; virtual;
  end;
...
{ TStrCollection object }
  ...
  TStrCollection = object(TSortedCollection)
    function  Compare(Key1, Key2: Pointer): Integer; virtual;
    procedure CopyFrom(Src: PStrCollection);
    procedure FreeItem(Item: Pointer); virtual;
    function  GetItem(var S: TStream): Pointer; virtual;
    procedure PutItem(var S: TStream; Item: Pointer); virtual;
  end;

{ TLineCollection object }
...
{ TSortedLineCollection object }

  PSortedLineCollection = ^TSortedLineCollection;
  TSortedLineCollection = object(TSortedCollection)
    function  Compare(Key1, Key2: Pointer): Integer; virtual;
    procedure FreeItem(Item: Pointer); virtual;
    function  GetItem(var S: TStream): Pointer; virtual;
    procedure PutItem(var S: TStream; Item: Pointer); virtual;
...
{ TDataCollection }
  ...
  TDataCollection = Object(TCollection)
    DataLen: Word;
    constructor Init(ALimit, ADelta: Integer; ADataLen: Word);
    constructor Load(var S: TStream);
...
{ TSortedDataCollection }
  ...
  PSortedDataCollection = ^TSortedDataCollection;
  TSortedDataCollection = Object(TSortedCollection)
    DataLen: Word;
    constructor Init(ALimit, ADelta: Integer; ADataLen: Word);
    constructor Load(Var S: TStream);
    procedure   CopyFrom(Src: PDataCollection);
...
{ ********************************************************
   Быстрые функции для работы с различными целыми типами.
  ******************************************************** }

function Min(X, Y: Integer): Integer;
inline($5A/$58/$3B/$C2/$7C/$01/$92);

function Max(X, Y: Integer): Integer;
inline($5A/$58/$3B/$C2/$7F/$01/$92);

function MinWord(X, Y: Word): Word;
inline($58/$5A/$3B/$C2/$76/$01/$92);

function MaxWord(X, Y: Word): Word;
inline($58/$5A/$3B/$C2/$73/$01/$92);


{ ***************************************
   Целый набор функций сравнения. Удобно
   использовать при сортировке коллекций
  *************************************** }
...
inline($5B/$5F/$07/  { POP     BX DI ES    }
       ...
       $23/$C3/      { AND     AX, BX      }
       $AB);         { STOSW               }
{ Аналогична X := X and not Flags (сброс флагов). И когда только Borland
  сделает X &= !Flags в Pascal'е ? }
...
{ Stream registration records }
  ...
  RSortedLineCollection: TStreamRec = (
    ObjType: 2502;
    VmtLink: Ofs(TypeOf(TSortedLineCollection)^);
    Load:    @TSortedLineCollection.Load;
    Store:   @TSortedLineCollection.Store);
  RDataCollection: TStreamRec = (
    ObjType: 2501;
    VmtLink: Ofs(TypeOf(TDataCollection)^);
    Load:    @TDataCollection.Load;
    Store:   @TDataCollection.Store);
  RSortedDataCollection: TStreamRec = (
    ObjType: 2503;
    VmtLink: Ofs(TypeOf(TSortedDataCollection)^);
    Load:    @TSortedDataCollection.Load;
    Store:   @TSortedDataCollection.Store);
...
function WordCmp(Key1, Key2: Word): Integer; assembler;
asm
        MOV     AX, Key1
        CMP     AX, Key2
        MOV     AX, 0
...
function LongCmp(Key1, Key2: LongInt): Integer; assembler;
asm
        MOV     AX, Key1.Word[0] { сравниваем старшие слова }
        CMP     AX, Key2.Word[0]
        MOV     AX, -1
        JL      @@1              { старшее слово содержит знаковый бит, ... }
        MOV     AX, 1
        JG      @@1
        MOV     AX, Key1.Word[2] { придется сравнить и младшие слова ОТДЕЛЬНО! }
        CMP     AX, Key2.Word[2] { нет чтобы MOV EAX, Key1 CMP EAX, Key2 и т.д.}
        MOV     AX, 0            { (только ради поддержки этого старья(286-х)) }
        JE      @@1              { кстати, могли бы и 386-й поддерживать эти }
        MOV     AX, -1           { загнивающие буржуи из Borland'а }
        JB      @@1              { ... а младшее сия участь миновала }
        MOV     AX, 1
@@1:
end;

function DWordCmp(Key1, Key2: LongInt): Integer; assembler;
asm
        MOV     AX, Key1.Word[0] { старшие слова }
        CMP     AX, Key2.Word[0]
@@1:    MOV     AX, -1
        JB      @@2
        MOV     AX, 1
        JA      @@2
        MOV     AX, Key1.Word[2] { а теперь и младшие (проклятый INTEL }
        CMP     AX, Key2.Word[2] { и вечная нехватка денег) }
        MOV     AX, 0            { XOR AX, AX испортит флаги }
        JNE     @@1              { непонятно, но место экономим (8 байт) }
@@2:
end;

procedure SwapPtr(var P1, P2: Pointer); assembler;
asm
        MOV     DX, DS
        CLD
        LDS     SI, P1
...
constructor TData.Init(ASize: Word);
begin
  inherited Init;
  Data := MemAllocSeg(ASize);
...
begin
  ...
  if Size > 0 then Data := MemAllocSeg(Size);
  if Data <> nil then S.Read(Data^, Size)
  else S.Seek(Size);
end;
...
function TStream.ReadPasString: String;
Var
  C: Char;
  S: String;
begin
  S := '';
  Read(C, 1);
  While (Status = stOk) and not (C in [#$0D, #27]) Do begin
    S := S + C;
    Read(C, 1);
  end;
  if C = #$0D then Read(C, 1);
  ReadPasString := S;
end;
...
begin
  Read(S, 1);
  Read(S[1], StrLength(S));
  ReadString := S;
end;

function TStream.ReadTextFile: PLineCollection;
Var
  P: PLineCollection;
begin
  ReadTextFile := nil;
  P := New(PLineCollection, Init(8, 4));
  While Status = stOk Do P^.Insert(StringRead);
  if Status <> stReadError Then Dispose(P, Done)
  else ReadTextFile := P;
end;

procedure TStream.Reset; assembler;
asm
        LES     DI, Self
        XOR     AX, AX
        MOV     ES:[DI].TStream.Status, AX
        MOV     ES:[DI].TStream.ErrorInfo, AX
end;
...
function TStream.StringRead: PString;
{Var
  C: Char;
  S: String;}
begin
{  S := '';
  ...
  if C = #$0D then Read(C, 1);}
  StringRead := NewStr(ReadPasString);
end;

procedure TStream.StringWrite(P: PString);
begin
  Write(P^[1], StrLength(P^));
  Write(CRLF, 2);
end;
...
procedure TStream.WritePasString(S: String);
begin
  Write(S[1], StrLength(S));
  Write(CRLF, 2);
end;
...
const
  Empty: String[1] = '';
begin
  if P <> nil then Write(P^, StrLength(P^) + 1) else Write(Empty, 1);
end;

procedure TStream.WriteString(S: String);
begin
  Write(S, StrLength(S) + 1);
end;
...
begin
  TStream.Init;
  Handle := 4; { стандартное устройство печати }
end;

destructor TPrinterStream.Done;
begin
  TStream.Done;
end;
...
asm
        LES     DI,Self
        CMP     ES:[DI].TBufStream.Status, stOk
        JNE     @@6
        MOV     AL, 1
        CALL    FlushBuffer
        JNE     @@6
...
constructor TXMSStream.Init(AMaxSize: Word); assembler;
asm
        XOR     AX,AX
        PUSH    AX
        LES     DI,Self
        ...
        MOV     DX, ES
        LES     DI, Self
        MOV     ES:[DI].TXMSStream.XMSAddr.Word[0], BX
        MOV     ES:[DI].TXMSStream.XMSAddr.Word[2], DX
        MOV     AH, 09H
        MOV     DX, AMaxSize
        CALL    ES:[DI].TXMSStream.XMSAddr
        OR      AX, AX
        JNZ     @@0
@@1:    MOV     DX, stInitError
        XOR     AX, AX
        MOV     AL, BL
        CALL    DoStreamError
        MOV     DX, -1
@@0:    MOV     ES:[DI].TXMSStream.Handle, DX
        MOV     AX, AMaxSize
        MOV     CX, 1024
        MUL     CX
        MOV     ES:[DI].TXMSStream.MaxSize.Word[0], AX
        MOV     ES:[DI].TXMSStream.MaxSize.Word[2], DX
end;
...
asm
        LES     DI, Self
        CMP     ES:[DI].TXMSStream.Status, stOk
        JNE     @@1
...
@@2:    PUSH    DS
        ...
        DEC     AX
@@21:   PUSH    AX
        MOV     [SI].TExtMemMove.Length.Word[2], BX
        MOV     [SI].TExtMemMove.Length.Word[0], AX
        MOV     AX, ES:[DI].TXMSStream.Handle
        MOV     [SI].TExtMemMove.SHandle, AX
        MOV     AX, ES:[DI].TXMSStream.Position.Word[0]
        MOV     DX, ES:[DI].TXMSStream.Position.Word[2]
        MOV     [SI].TExtMemMove.SOfs.Word[0], AX
        MOV     [SI].TExtMemMove.SOfs.Word[2], DX
        MOV     [SI].TExtMemMove.DHandle, BX
        LES     DI, Buf
        MOV     [SI].TExtMemMove.DOfs.Word[0], DI
        MOV     [SI].TExtMemMove.DOfs.Word[2], ES
        MOV     AH, 0BH
        LES     DI, Self
        CALL    ES:[DI].TXMSStream.XMSAddr
        POP     DX
        ...
        JZ      @@1
        XOR     BX, BX
        ADD     ES:[DI].TXMSStream.Position.Word[0], DX
        ADC     ES:[DI].TXMSStream.Position.Word[2], BX
        TEST    CX, 1
        JZ      @@3
        PUSH    DS
        PUSH    SS
        ...
        LEA     SI, S
        MOV     [SI].TExtMemMove.Length.Word[2], BX
        MOV     [SI].TExtMemMove.Length.Word[0], 2
        MOV     AX, ES:[DI].TXMSStream.Handle
        MOV     [SI].TExtMemMove.SHandle, AX
        MOV     AX, ES:[DI].TXMSStream.Position.Word[0]
        MOV     DX, ES:[DI].TXMSStream.Position.Word[2]
        ...
        LEA     DI, W
        MOV     [SI].TExtMemMove.DOfs.Word[0], DI
        MOV     [SI].TExtMemMove.DOfs.Word[2], SS
        MOV     AH, 0BH
        LES     DI, Self
        CALL    ES:[DI].TXMSStream.XMSAddr
        MOV     DX, W
        ...
        OR      AX, AX
        JZ      @@1
        ADD     ES:[DI].TXMSStream.Position.Word[0], 1
...
@@01:   LES     DI, Self
        ...
        JZ      @@11
        INC     CX
@@11:   MOV     [SI].TExtMemMove.Length.Word[2], BX
        MOV     [SI].TExtMemMove.Length.Word[0], CX
        MOV     [SI].TExtMemMove.SHandle, BX
        MOV     AX, ES:[DI].TXMSStream.Handle
        MOV     [SI].TExtMemMove.DHandle, AX
        MOV     AX, ES:[DI].TXMSStream.Position.Word[0]
        MOV     DX, ES:[DI].TXMSStream.Position.Word[2]
        MOV     [SI].TExtMemMove.DOfs.Word[0], AX
        MOV     [SI].TExtMemMove.DOfs.Word[2], DX
        LES     DI, Buf
        MOV     [SI].TExtMemMove.SOfs.Word[0], DI
        MOV     [SI].TExtMemMove.SOfs.Word[2], ES
        MOV     AH, 0BH
        LES     DI, Self
        CALL    ES:[DI].TXMSStream.XMSAddr
        POP     DS
        ...
        CALL    DoStreamError
        JMP     @@3
@@1:    MOV     AX, ES:[DI].TXMSStream.Position.Word[0]
        MOV     DX, ES:[DI].TXMSStream.Position.Word[2]
        XOR     BX, BX
        ADD     AX, Count
        ADC     DX, BX
        MOV     ES:[DI].TXMSStream.Position.Word[0], AX
        MOV     ES:[DI].TXMSStream.Position.Word[2], DX
        CMP     DX, ES:[DI].TXMSStream.Size.Word[2]
        JA      @@2
        JB      @@3
        CMP     AX, ES:[DI].TXMSStream.Size.Word[0]
        JBE     @@3
@@2:    MOV     ES:[DI].TXMSStream.Size.Word[0], AX
        MOV     ES:[DI].TXMSStream.Size.Word[2], DX
@@3:
end;
...
procedure TSortedCollection.Resort;
var
  I, J: Integer;
begin
  if Count > 1 then
    for I := 0 to Count - 2 do
      for J := I + 1 to Count - 1 do
        if Compare(KeyOf(Items^[I]), KeyOf(Items^[J])) = -1 then
          SwapPtr(Items^[I], Items^[J]);
end;
...
procedure TStrCollection.CopyFrom(Src: PStrCollection);
procedure CopyItem(P: PChar); far;
begin
  if P <> nil then Insert(StrNew(P))
  else Insert(nil);
end;
begin
  FreeAll;
  Src^.ForEach(@CopyItem);
end;
...
function TLineCollection.MaxLen;
var
  Max: Byte;
procedure MaxLenList(Item: PString); far;
begin
  if StrLength(Item^) > Max Then Max := StrLength(Item^);
end;
begin
  Max := 0;
  ForEach(@MaxLenList);
...
function TSortedLineCollection.Compare(Key1, Key2: Pointer): Integer;
begin
  Compare := StrCmp(PString(Key1)^, PString(Key2)^);
end;

procedure TSortedLineCollection.FreeItem(Item: Pointer);
begin
  DisposeStr(Item);
end;

function TSortedLineCollection.GetItem(var S: TStream): Pointer;
begin
  GetItem := S.ReadStr;
end;

procedure TSortedLineCollection.PutItem(var S: TStream; Item: Pointer);
begin
  S.WriteStr(Item);
end;

function TSortedLineCollection.MaxLen;
var
  Max: Byte;
procedure MaxLenList(Item: PString); far;
begin
...
begin
  S.Read(DataLen, SizeOf(Word));
  inherited Load(S);
end;

procedure TDataCollection.CopyFrom(Src: PDataCollection);
procedure CopyItem(P: Pointer); far;
var
  P1: Pointer;
begin
  if P <> nil then begin
    P1 := MemAlloc(DataLen);
    if P1 <> nil then Move(P^, P1^, DataLen);
    Insert(P1);
  end
  else Insert(nil);
end;
...
procedure TDataCollection.FreeItem;
begin
  if Item <> nil then FreeMem(Item, DataLen);
end;

function TDataCollection.GetItem;
var
  P: Pointer;
  IsNil: Boolean;
begin
  S.Read(IsNil, SizeOf(Boolean));
  if not IsNil then begin
    P := MemAlloc(DataLen);
    if P <> nil then S.Read(P^, DataLen);
  end else P := nil;
  GetItem := P;
end;

procedure TDataCollection.PutItem;
var
  IsNil: Boolean;
begin
  IsNil := Item = nil;
  S.Write(IsNil, SizeOf(Boolean));
  if not IsNil then S.Write(Item^, DataLen);
end;

procedure TDataCollection.Store;
begin
  S.Write(DataLen, SizeOf(Word));
  inherited Store(S);
end;

{ TSortedDataCollection }

constructor TSortedDataCollection.Init;
begin
  inherited Init(ALimit, ADelta);
  DataLen := ADataLen;
end;

constructor TSortedDataCollection.Load;
begin
  S.Read(DataLen, SizeOf(Word));
  inherited Load(S);
end;

procedure TSortedDataCollection.CopyFrom(Src: PDataCollection);
procedure CopyItem(P: Pointer); far;
var
  P1: Pointer;
begin
...
procedure TSortedDataCollection.FreeItem;
begin
  if Item <> nil then FreeMem(Item, DataLen);
end;

function TSortedDataCollection.GetItem;
var
  P: Pointer;
  IsNil: Boolean;
begin
...
procedure TSortedDataCollection.PutItem;
var
  IsNil: Boolean;
begin
  IsNil := Item = nil;
...
procedure TSortedDataCollection.Store;
begin
  S.Write(DataLen, SizeOf(Word));
  inherited Store(S);
end;
...
begin
  FreeMem(Item, StrLength(PResourceItem(Item)^.Key) +
...
begin
  S.Write(PResourceItem(Item)^, StrLength(PResourceItem(Item)^.Key) +
    (SizeOf(TResourceItem) - SizeOf(String) + 1));
end;
...
begin
  ...
  repeat
    Stop := True;
    if BasePos <= Stream^.GetSize - SizeOf(THeader) then begin
      Stream^.Seek(BasePos);
...
{$IFDEF NewExeFormat}
        ...
        $424E: if Header.InfoType = $3230 then begin  {Found Debug Info}
...
{$ELSE}
        ...
        $4246: if Header.InfoType = $5250 then Found := True
               else begin
...
{$ENDIF}
  ...
  if Stream^.Status <> stOk then Error;
end;
...
procedure TResourceFile.Error;
begin
  WriteLn('Error in resource file');
  Halt(3);
end;
...
var
  I: Integer;
begin
  if not Index.Search(@Key, I) then Get := nil
  else begin
...
var
  ...
  P: PResourceItem;
begin
  if Index.Search(@Key, I) then P := Index.At(I)
  else begin
    GetMem(P, StrLength(Key) + (SizeOf(TResourceItem) - SizeOf(String) + 1));
    P^.Key := Key;
    Index.AtInsert(I, P);
  end;
  P^.Pos := IndexPos;
...
function TResourceFile.SearchKey(Key: String): Boolean;
function DoSearchKey(P: PResourceItem): Boolean; far;
begin
  DoSearchKey := P^.Key = Key;
end;
begin
  SearchKey := Index.FirstThat(@DoSearchKey) <> nil;
end;
...
begin
  ...
  Inc(Cur.Count);
  Move(S, Strings^[StrPos], StrLength(S) + 1);
  Inc(StrPos, StrLength(S) + 1);
end;
...
procedure DisposeStr(P: PString);
begin
  if P <> nil then FreeMem(P, StrLength(P^) + 1);
end;
...
begin
  ...
  RegisterType(RSortedLineCollection);
  RegisterType(RDataCollection);
  RegisterType(RSortedDataCollection);
end;
