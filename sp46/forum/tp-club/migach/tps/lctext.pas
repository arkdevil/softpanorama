         ...
         {----------------------------------------------------}
         {   Модуль LcText  V 1.3.07 пакета  TURBO SUPPORT    }
         {     Язык программирования Turbo Pascal  V 6.0      }
         {----------------------------------------------------}
         { Дата последних изменений :  02/01/1991             }
         {----------------------------------------------------}
...
BEGIN
     ...
     {$IFDEF DEBUGTEXT}
     IF ( LnPtr^.Next <> NIL ) THEN
        ErrorInProc ( 'Ошибка в процедуре ClearListRecLine' );
     {$ENDIF}
     DISPOSE ( LnPtr );
     LnPtr := NIL
...
FUNCTION CopyListLine ( Quantity : LONGINT; LnPtr : RecLinePtr ) : RecLinePtr;
         ...
         { Передает уккказатель на дубликат списка }
VAR
   Hlp : RecLinePtr;
