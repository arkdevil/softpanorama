...
USES Dos, Crt, CheckFl, Printer, Def, JetFont, Fkey11, TWindow, MnCmd, Lptr,
     LcText, ViewT, ViewLt, EditT, MnBar, HLPrint;
...
VAR
   ...
   LineStartPage : STRING;
                { управляющие коды принтера в начале страницы }

   LineEndPage : STRING;
                { управляющие коды принтера в конце страницы }

   KeyLitl : BOOLEAN;
                { Признак узких букв }

   KeyCondens : BOOLEAN;
                { Признак Двойной печати }
   ...
   SingJet : BOOLEAN;
                { Признак наличия лазерного принтера }
...
BEGIN
     ...
     IF ( IORESULT = 0 ) THEN
        BEGIN
              ...
              Key := ( IORESULT <> 0 ) OR Key;
              BLOCKREAD ( Fl, LineStartPage, SIZEOF ( LineStartPage ) );
              Key := ( IORESULT <> 0 ) OR Key;
              BLOCKREAD ( Fl, LineEndPage, SIZEOF ( LineEndPage ) );
              Key := ( IORESULT <> 0 ) OR Key;
              BLOCKREAD ( Fl, KeyLitl, SIZEOF ( KeyLitl ) );
              Key := ( IORESULT <> 0 ) OR Key;
              BLOCKREAD ( Fl, KeyCondens, SIZEOF ( KeyCondens ) );
              Key := ( IORESULT <> 0 ) OR Key;
              ...
              Key := ( IORESULT <> 0 ) OR Key;
              BLOCKREAD ( Fl, SingJet, SIZEOF ( SingJet ) );
              Key := ( IORESULT <> 0 ) OR Key;
...
BEGIN
     ...
     Key := ( IORESULT <> 0 ) OR Key;
     BLOCKWRITE ( Fl, LineStartPage, SIZEOF ( LineStartPage ) );
     Key := ( IORESULT <> 0 ) OR Key;
     BLOCKWRITE ( Fl, LineEndPage, SIZEOF ( LineEndPage ) );
     Key := ( IORESULT <> 0 ) OR Key;
     BLOCKWRITE ( Fl, KeyLitl, SIZEOF ( KeyLitl ) );
     Key := ( IORESULT <> 0 ) OR Key;
     BLOCKWRITE ( Fl, KeyCondens, SIZEOF ( KeyCondens ) );
     Key := ( IORESULT <> 0 ) OR Key;
     ...
     Key := ( IORESULT <> 0 ) OR Key;
     BLOCKWRITE ( Fl, SingJet, SIZEOF ( SingJet ) );
     Key := ( IORESULT <> 0 ) OR Key;
...
BEGIN
     ...
     GWind^.XYPrint ( 18, 2, 'Программа обработки текстов и документов  V 3.3.J' );
     GWind^.SetColorFon ( MAGENTA );
     GWind^.SetColorSymbol ( LIGHTCYAN );
     GWind^.XYPrint ( 25, 23,'(c) 1990-1991,  Мигач Ярослав');
...
BEGIN
     ...
     CASE Num OF
           1000 : LineStartPage := Font1000;
           1001 : LineStartPage := Font1001;
           1002 : LineStartPage := Font1002;
           1003 : LineStartPage := Font1003;
           1004 : LineStartPage := Font1004;
           1005 : LineStartPage := Font1005;
           1006 : LineStartPage := Font1006;
           1007 : LineStartPage := Font1007;
           1008 : LineStartPage := Font1008;
           1009 : LineStartPage := Font1009;
           1010 : LineStartPage := Font1010;
           1011 : LineStartPage := Font1011;
           1012 : LineStartPage := Font1012;
           1013 : LineStartPage := Font1013;
           1014 : LineStartPage := Font1014;
           1015 : LineStartPage := Font1015;
           1016 : LineStartPage := Font1016;
           1017 : LineStartPage := Font1017;
           1018 : LineStartPage := Font1018;
           1019 : LineStartPage := Font1000;
           1020 : LineStartPage := Font1000
     END;
     LineEndPage := Font1000
...
BEGIN
     ...
     IF ( SingMod1 AND ( ( NowNumberPage MOD 2 ) = 0 ) AND
        ...
        EXIT;
     Hlp := Listing;
     LineToPrinter ( LineStartPage, GoToPrinter );
     ...
     IF ( GoToPrinter ) THEN
        EXIT;
     LineToPrinter ( LineEndPage, GoToPrinter );
...
BEGIN
     ...
     '  ..........................................................',
     BuildLineList (
     '  г. Киев  Тел  518-09-01 (дом); 441-40-81 (сл)',
     BuildLineList (
     '  Почтовый адрес: 253166 г. Киев ул. Милютенко 23 кв. 239',
     BuildLineList (
     '  ..........................................................',
     BuildLineList (
     '     Lprint является СВОБОДНО КОПИРУЕМЫМ программным продуктом',
     BuildLineList (
     ' Копирование программы, с целью получения коммерческой выгоды, ',
     BuildLineList (
     '            без согласования с автором, ЗАПРЕЩАЕТСЯ',
     NIL ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ), 100 ) );
     WITH HelpWindow DO
          BEGIN
...
BEGIN
    IF ( SingJet ) THEN
     ...
     Wn^.WPrint ( 3, 2, '   Опрелелите ширину букв ' );
     Wn^.FrameWindow ( 1, 1, 31, 5, 1, CHR ( 205 ) );
     Wn^.SetShade ( BLACK, BLACK );
     Wn^.PrintWindow;
     ...
     ELSE
         KeyLitl := FALSE;
     IF ( KeyLitl AND KeyCondens ) THEN
        BEGIN
             LineStartPage := OnLitl + OnCondens;
             LineEndPage := OffLitl + OffCondens
        END;
     IF ( NOT KeyLitl AND NOT KeyCondens ) THEN
        BEGIN
             LineStartPage := '';
             LineEndPage := ''
        END;
     IF ( NOT KeyLitl AND KeyCondens ) THEN
        BEGIN
             LineStartPage := OnCondens;
             LineEndPage := OffCondens
        END;
     IF ( KeyLitl AND NOT KeyCondens ) THEN
        BEGIN
             LineStartPage := OnLitl;
             LineEndPage := OffLitl
        END

END; { procedure SubCmd441 }
...
BEGIN
    ...
    ELSE
        command := 2;
    NEW ( Wn, MakeWindow ( 35, 17, 66, 22, MAGENTA, WHITE ) );
    Wn^.WPrint ( 3, 2, 'Опрелелите качество печати' );
    Wn^.FrameWindow ( 1, 1, 31, 5, 1, CHR ( 205 ) );
    Wn^.SetShade ( BLACK, BLACK );
    Wn^.PrintWindow;
     NEW ( Menu, SetMenu ( 45, 20, 59, 24, BLUE, WHITE, LIGHTRED, WHITE,
     ...
     NEW ( Menu,
         SetMenu ( 10, 8, 32, 22, BLUE, WHITE, LIGHTGRAY, BLACK,
           CYAN, BLACK, #205, TRUE, 1, HelpMenuBar,
              ...
              SetMenu ( 34, 13, 71, 16, BROWN, WHITE, BLACK, GREEN,
              CYAN, BLACK, #196, TRUE, 0, HelpMenuBar,
                    SetCmd ( 2, 2, ' Ширина символов ', 0, @SubCmd441,
                    SetCmd ( 20, 2,' Качество печати ', 0, @SubCmd442,
                    NIL ) ) ) ),
              ...
              CYAN, BLACK, #196, TRUE, 0, HelpMenuBar,
                 ...
                 SetCmd ( 3, 11,' Принтер          ', 0, @SubCmd49,
...
BEGIN
     WRITELN;
     WRITELN ( 'Lprint V 3.3.J  (c) 1990-1991, Мигач Ярослав' );
     WRITELN;

          {  Тест на изменения в исполняемом коде }
          CheckStart ( 'LPRINT.EXE' );
