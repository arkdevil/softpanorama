...
#ifdef __MSDOS__
FILE *popen(const char *command, const char *t);
int pclose(FILE *f);
static char *popen_cmd;
#endif __MSDOS__
...
begin_output ()
{
  ...
  if (paginate_flag)
    {
#ifdef __MSDOS__
      popen_cmd = (char *) xmalloc (strlen (PR_FILE_NAME) + 10 + strlen(name));
      strcpy(popen_cmd, PR_FILE_NAME);
      strcat(popen_cmd, " -f -h \"");
      strcat(popen_cmd, name);
      strcat(popen_cmd, "\"");

      if ((outfile = popen(popen_cmd, "w")) < 0)
...
print_1_line (line_flag, line)
     ...
     const char HUGE * const *line;
{
  const char HUGE *text = line[0], HUGE *limit = line[1]; /* Help the compiler.  */
...
void
output_1_line (text, limit, flag_format, line_flag)
     const char HUGE *text, HUGE *limit, *flag_format, *line_flag;
{
  if (!tab_expand_flag)
  ...
  else
    {
      ...
      register char c;
      register const char HUGE *t = text;
