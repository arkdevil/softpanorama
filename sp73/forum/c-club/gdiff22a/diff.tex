...
@subtitle Edition 1.1, for @code{diff} 2.1 and @code{patch} 2.0.12g8
...
This is Edition 1.0, for @code{diff} 2.1 and @code{patch} 2.0.12g8.
@end ifinfo
...
in ``An O(ND) Difference Algorithm and its Variations'', Eugene Myers,
@cite{Algorithmica} Vol. 1 No. 2, 1986, p. 251; and in ``A File
Comparison Program'', W. Miller and E. Myers, @cite{Software Practice
and Experience} Vol. 15 No. 11, 1985, p. 1025.
...
differences are changes in the amount of whitespace between words or
...
changes in both whitespace and alphabetic case.
...
two files are identical.  For this purpose, it is better than
@code{diff} because it is much faster.
...
* Whitespace::		Suppressing differences in whitespace.
...
@node Hunks, Whitespace, , Comparison
@section Hunks
@cindex hunks
...
@node Whitespace, Blank Lines, Hunks, Comparison
...
The @samp{-b} and @samp{--ignore-space-change} options ignore blanks and
tabs at line end, and to consider all other sequences of one or more
blank and tab characters to be equivalent.  With these options,
...
@samp{-b}.  They ignore difference even if one file has whitespace where
the other file has none, and they ignore all whitespace characters, not
just blanks and tabs.  (The @dfn{whitespace} characters include
backspace, tab, vertical tab, formfeed, carriage return, space, and
no-break space.)  With these options, @code{diff} considers the
...
@node Blank Lines, Case Folding, Whitespace, Comparison
...
This option automatically selects the context output format
(@pxref{Context Format}), with the default number of lines of context.
You can override that number with @samp{-C @var{lines}} later in the
command line.  You can override both the format and the number with
@samp{-U @var{lines}} later in the command line.
...
@samp{--ignore} options (@pxref{Whitespace}).
...
it is specified; otherwise it a format that outputs the line group as-is.
...
@samp{--ignore-whitespace} option (@pxref{Changed Whitespace}, for more
information).
...
came from.  Two spaces precede each copy of an input line to
distinguish it from the commands.  Commands take the following forms:
...
entire diff is indented by a constant amount of whitespace, @code{patch}
...
@menu
* Changed Whitespace::	When tabs and spaces don't match exactly.
* Reversed Patches::	Applying reversed patches correctly.
* Inexact::		Helping @code{patch} find close matches.
@end menu

@node Changed Whitespace, Reversed Patches, , Imperfect
@subsection Applying Patches with Changed Whitespace
@cindex whitespace in patches
...
@samp{--ignore-whitespace} option, which makes @code{patch} compare
whitespace loosely so that any sequence of whitespace in the patch file
matches any sequence of whitespace in the input files.  Non-whitespace
characters must still match exactly.  Each line of the context must
still match a line in the input file.

@node Reversed Patches, Inexact, Changed Whitespace, Imperfect
...
@itemize @bullet
@item
skip patches for which it can't find a file to patch (the same as @samp{-f});
...
whitespace or a newline.  If not, @code{patch} prompts you for
...
produced by giving @code{diff} the @samp{-C 2} option.  Give @code{diff}
the @samp{-N} option in case the newer version of the package contains
any files that the older one does not.  Make sure to specify the scratch
directory first and the newer directory second.
...
(@pxref{Unified Format}).  This option is obsolete.
...
@item -b
Ignore changes in amount of blank and tab whitespace.  @xref{Whitespace}.
...
Ignore whitespace when comparing lines.  @xref{Whitespace}.
...
@item -w
Ignore horizontal whitespace when comparing lines.  @xref{Whitespace}.
...
@code{patch} strips off all leading paths, leaving just the base file
names, except that when a file name given in the patch is a relative
path and all of its leading directories already exist, @code{patch} does
not strip off the leading path.  (A @dfn{relative} path is one that does
...
@itemx -l
Let any sequence of whitespace in the patch file match any sequence of
whitespace in the input file.  @xref{Changed Whitespace}.
...
differences in blank lines are ignored (@pxref{Blank Lines}).
...
@cindex directory structure shanges
...
sockets.  Currently, @code{diff} treats all these files like regular
files.  However, this means that @code{patch} cannot represent changes
...
whitespace, @samp{diff -r} generates a patch that @code{patch} cannot
