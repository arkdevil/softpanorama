...
print_1sdiff_line (left, sep, right)
     const char HUGE * const *left;
     int sep;
     const char HUGE * const *right;
{
  FILE *out = outfile;
     ...
     int limit0, limit1;
{
  ...
  if (! sdiff_skip_common_lines  &&  (i0 != limit0 || i1 != limit1))
      ...
      if (! sdiff_left_only)
	{
	  ...
	  while (i1 != limit1)
	    print_1sdiff_line (NULL, ')', &files[1].linbuf[i1++]);
	}
      while (i0 != limit0)
	print_1sdiff_line (&files[0].linbuf[i0++], '(', NULL);
    }
     ...
     struct change *hunk;
{
  ...
  if (inserts)
	...
	print_1sdiff_line (NULL, '>', &files[1].linbuf[j]);
      next1 = j;
    }
  ...
  if (deletes)
	...
	print_1sdiff_line (&files[0].linbuf[i], '<', NULL);
      next0 = i;
    }
}
