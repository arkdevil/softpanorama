...
MENU *menu0;
int  attr0[] ={LIGHTGRAY on BLACK,LIGHTGRAY on BLACK,BLACK on CYAN,WHITE on BLACK,DARKGRAY on LIGHTGRAY,CYAN on BLUE,LIGHTBLUE on BLACK,DARKGRAY on LIGHTGRAY,DARKGRAY on LIGHTGRAY};
int  attr1[] ={LIGHTGRAY on BLACK,LIGHTGRAY on BLACK,LIGHTGRAY on BLACK,WHITE on BLACK,DARKGRAY on LIGHTGRAY,CYAN on BLUE,LIGHTBLUE on BLACK,LIGHTBLUE on BLUE,DARKGRAY on LIGHTGRAY};
int  attr2[] ={LIGHTGRAY on BLACK,LIGHTGRAY on BLACK,BLACK on CYAN,WHITE on BLACK,DARKGRAY on LIGHTGRAY,CYAN on BLUE,LIGHTBLUE on BLACK,LIGHTBLUE on BLUE,DARKGRAY on LIGHTGRAY};
int  attr3[] ={LIGHTGRAY on BLACK,LIGHTGRAY on BLACK,BLACK on CYAN,WHITE on BLACK,DARKGRAY on LIGHTGRAY,CYAN on BLUE,LIGHTBLUE on BLACK,DARKGRAY on LIGHTGRAY,LIGHTBLUE on BLACK};
int  attr4[] ={DARKGRAY on LIGHTGRAY,LIGHTGRAY on BLACK,BLACK on CYAN,DARKGRAY on LIGHTGRAY,WHITE on LIGHTGRAY,CYAN on BLUE,WHITE on LIGHTGRAY,LIGHTBLUE on BLUE,WHITE on BLUE};
int  attr5[] ={LIGHTGRAY on BLACK,LIGHTGRAY on BLACK,BLACK on LIGHTGRAY,LIGHTBLUE on BLACK,DARKGRAY on LIGHTGRAY,CYAN on BLUE,LIGHTBLUE on BLACK,LIGHTBLUE on BLUE,WHITE on BLUE};
char frame0[] ="╔═╗║╝═╚║";
char frame1[] ="╦═╦║╝═╚║";
char frame2[] ="┌─┐│┘─└│╔═╗║╝═╚║";
char frame3[] ="";
...
int  flag1   = AREASAVE | BORDURE | WINDOW | RECOME;
...
menu0 =submenu(" Это Демонстрационное Меню ",frame0,wind0,attr0,25,flag0);
         menuxy(14,1);
         lenmenu(59,3);
         menuitem("Опция 1", 16, 2, 7, 1, "Это подсказка к Опции 1");
       submenu("",frame1,wind0,attr1,25,flag1);
         helpxy(CENTRCENTR,25);
           menuitem(" Опция 11 ", 16, 4, 9, 6, "Строка-подсказка к Опции 1");
           menuitem(" Опция 12 ", 16, 5, 9, 7, "А это уже строка-подсказка к Опции 12");
           menuitem(" Опция 13 ", 16, 6, 9, 8, "У Опции 14 строки-подсказки НЕТ !");
           menuitem(" Опция 14 ", 16, 7, 9, 9, "");
           menuitem(" Опция 15 ", 16, 8, 9, 10, "В даном подменю работает АВТОСДВИГ !!!");
           menuitem(" Опция 16 ", 16, 9, 9, 11, "Строка-подсказка центрована в ЦЕНТР");
       popsubmenu();
         menuitem("Опция 2", 28, 2, 7, 2, "А это уже подсказка к Опции 2");
       submenu("",frame0,wind0,attr2,5,flag1);
         helpxy(41,5);
           menuitem(" Опция 21 ", 27, 4, 9, 12, " Это строка-подсказка к опции 21 ");
           menuitem(" Опция 22 ", 27, 5, 9, 13, " А это уже строка подсказки к Опции 22");
           menuitem(" Опция 23 ", 27, 6, 9, 14, " Подсказка к Опции 24 отсутствует !!!");
           menuitem(" Опция 24 ", 27, 7, 9, 15, "");
       popsubmenu();
         menuitem("Опция 3", 40, 2, 7, 3, "У Опции 4 строки-подсказки НЕТ !");
       ...
       popsubmenu();
         menuitem("Опция 5", 64, 2, 7, 5, "Выход производится по нажатию клавиши ESC");
