...
Get_IRQ_DMA_ScanName proc near
	mov	data_162,0
	mov	data_164,1
	cmp	ScanModeIndex,0008
	JNL	@@1
	or word ptr ds:data_164,4
@@1:
	mov	ax,IObase
	mov	copyIObase,AX
	mov	ax,dIObase3
	mov	copydIObase3,AX
	mov	al,IRQnumber
	mov	copyIRQnumber,AL
	mov	al,DMAchannel
	mov	copyDMAchannel,AL
	cld
	mov	si,offset data_160
	rep	movsb	;	длина стpуктуpы 29h байт
	ret
endp
...
StartScan proc near
		...
		add	di,0Eh
		cld
		stosw	; обнулим счетчик стpуктуp
		stosw
		stosw
		pop	di
		...
		pop	ds es	; пpи этом заполняется u_mask и Pointer0
		rep	movsb
		pop	di es si ds
