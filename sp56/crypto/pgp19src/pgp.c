...
{	
	...
	{	/* temporary scope for some buffers */
		...
		certificate_length = make_signature_certificate(certificate, &MD, n, d, p, q, u);
	...
	*certificate++ = ctb;	/* copy ctb into certificate */
	...
	{	separate_signature = FALSE;
		...
		{	/* skip over the CTB_LITERAL header to compute signature */
			getpastlength(ctb2, f); /* read packet length */
			start_text = ftell(f);	/* mark position of text for later */
...
int bass_encryptfile(char *infile, char *outfile)
{
	FILE *f;	/* input file */
	FILE *g;	/* output file */
	byte basskey[256];
...
int encryptfile(char *mcguffin, char *infile, char *outfile)
{
	...
	int i,blocksize,ckp_length,PKElength;
	FILE *f;
	FILE *g;
	unit n[MAX_UNIT_PRECISION], e[MAX_UNIT_PRECISION];
	byte inbuf[MAX_BYTE_PRECISION];
...
int decryptfile(char *infile, char *outfile)
{
	...
	getpastlength(ctbCKE, f); /* read packet length */
...
{	char keyfile[64], plainfile[64], cipherfile[64], ringfile[64], tempfile[64];
	...
	if (argv[1][0] == '-')
	{
		...
		{	/* Sign AND encrypt file */
			...
			status = encryptfile(mcguffin, SCRATCH_CTX_FILENAME, cipherfile );
		...
		{	/*	Encrypt file
			...
			*/
			...
			status = encryptfile(mcguffin, plainfile, cipherfile );
		...
		{	/*	Encrypt file with BassOmatic only
			...
			*/
			...
			status = bass_encryptfile( plainfile, cipherfile );
