         ...
         {----------------------------------------------------}
         { Дата последних изменений :  13/05/1992             }
         {----------------------------------------------------}
         {   Модуль определений и низкоуровнево ввода         }
         {----------------------------------------------------}
         { (c) 1990-1992, Ярослав Мигач                       }
         {----------------------------------------------------}
...
TYPE
     ...
     TextScreen = ARRAY [ 1..25, 1..80 ] OF RECORD
                                                 ...
                                                 at : BYTE
                                            END;
        { Текстовый экран 80X25 }

     AnyBuffer = ARRAY [ 1..$FFFE ] OF BYTE;
        { Абстрактный буффер }

     AnyBufferPtr = ^AnyBuffer;
        { Указатель на абстрактный буффер }
...
PROCEDURE ReadText ( VAR Line : STRING );
...
PROCEDURE SetMaxBuf ( Count : LONGINT );
...
BEGIN
     IF ( ( SizeBuf ) < IndBuf ) THEN
        BEGIN
             BLOCKREAD ( Ft, BufferPtr^, MaxBuf, SizeBuf );
             Error := IORESULT;
             IF ( SizeBuf = 0 ) THEN
                BEGIN
                     EofKey := 1;
             ...
             IndBuf := 1;
        END;
     NextChar := CHR ( BufferPtr^ [ IndBuf ] - PasswordText );
     INC ( IndBuf )
...
BEGIN
     IF ( Count >= 518 ) THEN
        BEGIN
             IF ( Count < $FFFE ) THEN
                MaxBuf := Count MOD $FFFE
             ELSE
                 MaxBuf := $FFFE
        END

END; { PROCEDURE SetMaxBuf }
