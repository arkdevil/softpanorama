 ...
 *
 * $Log:	vars.c,v $
 * Revision 1.8  92/09/02  18:37:02  ache
 * MSDOS: value изменено, чтобы не хватала лишнюю память
 * 
 * Revision 1.7  92/08/24  02:23:32  ache
...
/*NOXSTR*/
static char rcsid[] = "$Header: vars.c,v 1.8 92/09/02 18:37:02 ache Exp $";
/*YESXSTR*/
	...
	char name[], val[];
{
	...
	if (equal(name,"MSG"))
		_ediag = (!*val || index("РрPpRr", *val) == NOSTR);
	else if (equal(name, "debug"))
		debug = 1;
	else if (equal(name,"separate-news") && value("sort") == NOSTR)
		assign("sort", "file");
}
