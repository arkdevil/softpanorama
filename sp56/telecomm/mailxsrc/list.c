 ...
 *
 * $Log:	list.c,v $
 * Revision 1.11  93/01/04  02:15:55  ache
 * I decide to put all changes into RCS now
 * 
 * Revision 1.10  92/08/24  02:20:08  ache
 * Основательные правки и перенос в разные системы
 * 
 * Revision 1.10  1991/07/17  19:56:23  ache
 * Теперь вектор сообщений формируется в порядке их указания
 *
...
/*NOXSTR*/
static char rcsid[] = "$Header: list.c,v 1.11 93/01/04 02:15:55 ache Exp $";
/*YESXSTR*/
...
"Ссылка перед началом списка писем\n"));
					return(-1);
				}
			}
			break;
...
first(f, m)
{
	...
	for (mp = dot, mesg = dot - message + 1; mesg <= msgCount; mp++, mesg++) {
		if ((mp->m_flag & m) == f)
			return(mesg);
	}
	for (mp = dot-1, mesg = dot - message; mesg > 0; mp--, mesg--) {
		if ((mp->m_flag & m) == f)
			return(mesg);
	}
	return((int)NULL);
	...
	char *str;
{
	...
	return instr(upperstr(cp), upperstr(str));
}
	...
	char *str;
{
	...
	cp = str;
	if ((cp2 = hfield("subject", mp)) == NOSTR)
		cp2 = hfield("subj", mp);
	if (cp2 == NOSTR)
		return(0);
	backup = cp2;
...
unmark(mesg)
{
	...
	if (i < 1 || i > msgCount)
		panic(ediag("Bad message number to unmark",
			    "Плохой номер письма для unmark"));
	message[i-1].m_flag &= ~MMARK;
}
