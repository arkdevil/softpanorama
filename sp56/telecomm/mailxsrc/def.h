 ...
 *
 * $Log:	def.h,v $
 * Revision 1.12  93/01/04  02:14:36  ache
 * I decide to put all changes into RCS now
 * 
 * Revision 1.11  92/08/24  02:17:29  ache
 * Основательные правки и перенос в разные системы
 * 
 * Revision 1.19  1992/05/17  14:48:46  ache
 * Fine port to Ultrix
 *
...
/*
 * $Header: def.h,v 1.12 93/01/04 02:14:36 ache Exp $
 */
...
#define SEPCHAR '/'
#ifdef ultrix
#define sigtype void    /* bug in 'cc', can't handle typedefed */
#else   /* !ultrix */
#if defined(M_SYSV) && !defined(M_XENIX) || defined(sun) || defined(__386BSD__)
typedef void sigtype;
#else   /* !M_SYSV || M_XENIX */
typedef int sigtype;
#endif  /* !M_SYSV || M_XENIX */
#endif  /* !ultrix */
#endif  /* !MSDOS */
...
#ifndef MSDOS
#if !defined(M_SYSV) && !defined(sun) && !defined(__386BSD__)
#include <sgtty.h>
#define USE_SGTTY
#else
...
#define stty(f, s) tcsetattr(f, TCSDRAIN, s)
#else
#include <termio.h>
#define USE_TERMIO
...
#ifdef  VMUNIX
#define VBUF(file,buf,size) setbuffer(file,buf,size)
#else
#if defined(M_XENIX) && !defined(M_I386)  /* Broken XENIX286 setvbuf... */
#define VBUF(file,buf,size) setvbuf(file,_IOFBF,buf,size)
#else
#define VBUF(file,buf,size) setvbuf(file,buf,_IOFBF,size)
#endif
#endif
...
#define LINESIZE        1024            /* max readable line width */
...
#define GMASK   (GTO|GSUBJECT|GCC|GREFS|GADD|GNGR|GNEWS|GIRTO)
...
#define GIRTO   2048            /* Add In-Reply-To: line */
...
struct header {
	char    *h_to;                  /* Dynamic "To:" string, result */
	char    *h_cc;                  /* Carbon copies string, result */
	char    *h_bcc;                 /* Blind carbon copies, result */
	char    *h_to_template;         /* Dynamic "To:" string, user defined */
	char    *h_cc_template;         /* Carbon copies string, user defined */
	char    *h_bcc_template;        /* Blind carbon copies, user defined */
	char    *h_subject;             /* Subject string */
	char    *h_refs;                /* References to messages */
	char    *h_inreplyto;           /* In-Reply-To field */
...
struct name {
	...
	char    *n_address;             /* This fella's address */
};
...
#define SF_BINARY       8       /* Binary mode (MSDOS) */

/*
 * Defenitions for sender's name
 */
#define FOR_display 0   /* Real sender name */
#define FOR_reply   1   /* One address */
#define FOR_Reply   2   /* Multiply adresses, skin <...> */
