 ...
 *
 * $Log:	head.c,v $
 * Revision 1.9  93/01/04  02:15:36  ache
 * I decide to put all changes into RCS now
 * 
 * Revision 1.8  92/08/24  02:19:49  ache
 * Основательные правки и перенос в разные системы
 * 
 * Revision 1.13  1991/12/25  00:07:28  ache
 * Убран излишний интеллект в распознавании заголовка
 *
...
/*NOXSTR*/
static char rcsid[] = "$Header: head.c,v 1.9 93/01/04 02:15:36 ache Exp $";
/*YESXSTR*/
static void fail();
...
register int c;
{
	...
	return (c >= ' ' && c < 0177) ||
#ifndef MSDOS
	       (c >= 0x80 && c != 0x93 && c != 0x9b)
#else
	       (c >= 0x80 && c != 0xf4 && c != 0xf5)
#endif
	;
}
...
{
	c &= 0xff;
	return (isprint(c) || c == 0177 || c && index("\7\b\f\n\r\t", c) != NOSTR);
}
...
static unsigned char Ctok(c) register unsigned char c;
{return (c&0300)==0300?_utok[c&077]:c;}
static unsigned char Ctou(c) register unsigned char c;
{return (c&0300)==0300?_ktou[c&077]:c;}

int lcmp(c1, c2) register unsigned char c1, c2;
{return (int)Ctou(c1) - (int)Ctou(c2);}
#else   /* MSDOS */
int lcmp(c1, c2) register unsigned char c1, c2;
...
{
	register char *s;
	register int slen, plen;

	if (str == NOSTR || pat == NOSTR)
		return 0;
	if (!*str && !*pat)
		return 1;
	if (!*str)
