...
__r_flush	PROC FAR
	...
	CLI
	MOV	SIZE_RDATA[SI],0 ; CLEAR NUMBER OF BYTES USED
	MOV	START_RDATA[SI],0
	MOV	END_RDATA[SI],0
...
RECV3:  CMP URGENT_SEND[SI],1           ; Previous send still in progress?
...
SEND1:	CMP     SIZE_TDATA[SI],S_SIZE-1 ; BUFFER FULL? (Leave room for SENDII)
		...
		CLI
		MOV	BX,END_TDATA[SI]	; BX POINTS TO FREE SPACE
...
SENDI3: CMP URGENT_SEND[SI],1           ; Previous send still in progress?
...
SENDII  PROC NEAR
		...
		MOV BX,START_TDATA[SI]          ; 1st char
		MOV	TDATA[SI][BX],AL	; CLOBBER FIRST CHAR IN BUFFER
		JMP SHORT SENDII4

SENDII2:MOV BX,START_TDATA[SI]          ; DX is index of 1st char
		DEC BX                          ; Back it up
		AND BX,S_SIZE-1                 ; Ring it
		MOV TDATA[SI][BX],AL            ; Move character to buffer
		MOV START_TDATA[SI],BX          ; Save new value
		INC     SIZE_TDATA[SI]          ; ONE MORE CHARACTER IN X-MIT BUFFER
...
__set_connected	PROC FAR
	...
	CMP	CONNECTION[SI],'D' ; MODEM CONNECTION?
	JE	SCE
	MOV	CONNECTED[SI],1

SCE:POP	SI				; RECOVER REGS
	mov sp,bp
	pop bp
	RET				; DONE
...
MSI:    MOV DX,MSR[SI]                  ; Modem Status Register
		...
		CMP	AL,8		; DDCD && !DCD
		 JNE MS00
		MOV	CARRIER[SI],0	; mark LOST
		JMP	SHORT MS01

MS00:	CMP	AL,88H
		 JNE MS01
		MOV	CARRIER[SI],1	; mark PRESENT
MS01:
		MOV	AL,BL
...
TX_CHR  PROC NEAR
		MOV BX,START_TDATA[SI]
		MOV AL,TDATA[SI][BX]            ; Get character from buffer
