...
/*--------------------------------------------------------------------*/
   ...
   if (hostp->aliased)
   {
      ...
      {
		 printmsg(0,"HostAlias: Alias table loop detected with host %s",
               hostp->hostname);
      }
...
/*--------------------------------------------------------------------*/
   ...
   if (hostp->routed)
   {
      if ( hostp->via == NULL )
      {
		 printmsg(0,"HostPath: Routing table loop discovered at host %s",
                     hostp->hostname);
         panic();
      }
   ...
   if ( hostp->via == NULL )
   {
      ...
      if (equal(hostp->hostname,alias))
      {
         if (hostp->hstatus == localhost) /* Ourself?                */
			hostp->via = nodename;      /* Yes --> Deliver local   */
      ...
      switch (state) {
         ...
         case ')':
            ...
            switch(*column) {
               ...
               case '>':
               case ')':
				  printmsg(0,"ExtractAddress: Invalid RFC-822 address: %s",nonblank);
