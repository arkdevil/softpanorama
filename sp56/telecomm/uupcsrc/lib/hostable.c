...
{
   static char  hostname[MAXADDR];   /* Local copy of name to process       */
...
/*--------------------------------------------------------------------*/
   ...
   token = strrchr(domain,'.');
   if (token == NULL)
   {
	  printmsg(0,"loadhost: domain name \"%s\" is invalid, missing period",domain);
	  panic();
   }
...
/*--------------------------------------------------------------------*/
   ...
   hostp->realname = nodename; /* Don't let user alias our system
								  name                             */
...
/*--------------------------------------------------------------------*/
   ...
   if (hostp->via == NULL )   /* Not initialized?                    */
	  hostp->via      = nodename;  /* Correct --> Route via local  */
   else
	  panic();           /* "Houston, we a have problem" -
							 Apollo 13                           */
   hostp->realname = nodename;
...
/*--------------------------------------------------------------------*/
   ...
   if (ff == NULL)
   {
	  printerr("loadhost", s_systems);
	  printmsg(0, "loadhost: can't open %s file", s_systems);
	  panic();
   }
   ...
   while (! feof(ff)) {
	  ...
	  if (hostp->hstatus == phantom)
	  {
		 ...
		 printmsg(5,"loadhost: duplicate entry in %s for \"%s\" ignored",
			   s_systems,
			   hostp->hostname);
   } /* while */
...
/*--------------------------------------------------------------------*/
	 ...
	 "loadhost: wildcard \"%s\" not allowed for real name of host \"%s\"",
	  ...
	  printmsg(5,"loadhost: cannot open host routing file %s",s_systems);
...
/*--------------------------------------------------------------------*/
   ...
   if (fdomain != NULL)     /* If fromdomain provided ...          */
   {
	  hostp = inithost(fdomain);

	  if (hostp->via == NULL)    /* Uninitialized?                   */
		 hostp->via = mailserv;   /* Yes --> Use default route     */
   }
   else
...
/*--------------------------------------------------------------------*/
   ...
   if ( localdomain == NULL )
   {
	  ...
	  printmsg(5,"loadhost: local domain defined as \"%s\"",localdomain);
   } /* if */
