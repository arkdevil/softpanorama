...
static char  grade = 'M';          /* Default grade of service */
...
\t[-aNAME] [-z] [-] [-x|-X DEBUG_LEVEL] [-l|command-string]\n");
...
/*--------------------------------------------------------------------*/
   ...
   while ((c = getopt(argc, argv, "-a:bcCEejg:lnprs:x:X:z")) !=  EOF)
	  switch(c) {
	  case 'l':
		 filelist = TRUE;
		 break;
	  ...
	  case 'x':
		 ...
		 break;
	  default:
		 fprintf(stderr, "uux - bad argument from getopt \"%c\"", c);
	  case '?':
		 usage();
   }
