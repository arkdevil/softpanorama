page ,132
title fd ( find directory ) as of 05/05/96 - 01:30 am
;*-------------------------------------------------------
;*
;*   this program will Find [the specified] Directory
;*
;*   and display its full drive / path
;*
;*   syntax :  fd dirname
;*
;*-------------------------------------------------------
...
z        equ   0                       ; zero
lf       equ   10                      ; line feed
mfnl     equ   13                      ; max file name length
...
idnl     db    z                       ; input dir name len

sodn     db    z                       ; save old drv # len
...
sem1     db    ' * FD error !',z
sem1a    db    ' * syntax = :',z
sem2     db    ' * FD dirname ',z
sem3     db    ' * dirname = name of directory to find ',z

cdem     db    ' * change directory error !',z

rnnfm    db    ' * directory name not found !',z
...
;*------------------------------
         ...
         mswl  dpn,clna,idnl           ; move cmd line name
...
init:
         ...
         call  scdap                   ; save cuurent drv and path

         mov   drvltr,'c'              ; drive ltr = 'c'
         mov   dn,2                    ; drive no  = 'c'

         sdta  dea                     ; set disk transfer address
...
chdir:
         ...
         cmp   ax,0                    ; chk result
         jne   ckax                    ; if any err, check for possibles

         call  rdap                    ; reset drv and path
...
rnnf:
         lm    crlf                    ; cr, lf
         lm    rnnfm                   ; rec not fnd msg
         lm    crlf
         lm    dnm                     ; dir name msg
         lm    dpn                     ; dir path name
         lm    crlf
         jmp   rtde                    ; jmp to err exit
