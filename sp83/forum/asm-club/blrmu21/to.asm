page ,132
title TO ( chg TO drv and dir ) as of 05/08/96 - 11:35 pm
;*---------------------------------------------------------
;
;
;        TO ( chg TO drv and dir )
;
;        syntax :
;                 to x ddddddddd
;
...
;*---------------------------------------------------------
;
;        also, calls cyan.prc which clears screen and
;        changes color to white on cyan
;
;*---------------------------------------------------------
...
pcl:
         cmp   pl,0          ; parm len = 0 ?
         je    jtsx          ; if EQ, get out
         cmp   pl,ml         ; min limit ?
         jb    jtsx          ; if LT, get out
         mov   al,pl         ; else, save
         mov   spl,al        ; parm len
         jmp   gdl           ; and carry on
...
;*-------------------
         ...
         ccd   dpd           ; change directory
         jc    dem           ; if carry set, err
         jmp   sx            ; else, jump to std exit
...
cyan     proc  near
         ...
         mov   bh,0                    ; page number 0
         mov   bl,63       ; x'3f'     ; attr = cyan back, white crsr
         mov   cx,2000                 ; 25 * 80
         int   16
