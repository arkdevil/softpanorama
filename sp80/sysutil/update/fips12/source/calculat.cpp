/*
	...
	RCS - Header:
	$Header: c:/daten/fips/source/main/RCS/calculat.cpp 1.1.1.1 1994/10/13 01:53:16 schaefer Exp schaefer $
...
void fips_partition_table::calculate_new_root (dword new_start_cylinder,partition *partition,const drive_geometry &geometry)
{
	...
	partition_info[i].end_cylinder = partition_info[partition_no].end_cylinder % 1024;
	partition_info[i].start_sector = 1;
	partition_info[i].start_head = 0;
	partition_info[i].start_cylinder = new_start_cylinder % 1024;
	...
	partition_info[partition_no].end_cylinder = (new_start_cylinder - 1) % 1024;
