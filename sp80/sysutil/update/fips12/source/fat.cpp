/*
	...
	RCS - Header:
	$Header: c:/daten/fips/source/main/RCS/fat.cpp 1.1.1.1 1994/10/13 01:53:28 schaefer Exp schaefer $
...
dword fat16::next_cluster (dword cluster_number)
{
	...
	if (sector != sector_in_buffer) read_sector (sector);
		...
		if (i == 0)
		{
			if (buffer->data[0] != 0xf8)
				if (!global.override_media_descriptor)
					error ("Wrong Media Descriptor Byte in FAT: %02Xh",buffer->data[0]);
			if ((buffer->data[1] != 0xff) || (buffer->data[2] != 0xff) || (buffer->data[3] != 0xff))
				warning ("Wrong FAT entries 1 & 2: %02X %02X %02X %02X",buffer->data[0],buffer->data[1],buffer->data[2],buffer->data[3]);
		}
	}
	printx ("OK\n");
}
