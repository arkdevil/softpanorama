 ...
 только после проведения теста "дискета-дрозофила" (см.прил.) к  секре-
 тарю выпуска или Максиму Круковскому 221-2148,  449-9767 (д). Я убеди-
 тельно прошу придерживаться принципа
...
*********** ТЕСТ НА НАЛИЧИЕ ВИРУСОВ С ПОМОЩЬЮ ДИСКЕТЫ-ДРОЗОФИЛЫ **********
 ...
 раженности компьютера.  Наиболее простым и надежным представляетя раз-
 работанный автором тест, получивший название "ловлей вируса на дискету
 -дрозофилу".
      Дискетой-дрозофилой будем называть копию  некоторой  загружаеёмой
 дискеты  с  MS  DOS той версии,  которая используемтся на компьютере и
 ...
 для которой ОБЯЗАТЕЛЬHО существует подготовленная  заранее  эталонная,
 ...
 бы "вытаскивается на свет".  Hапример,  место, куда вирус записал свое
 ...
 операционной системой (т.е.  с эталонной копии дискетыдрозофилы, в на-
 шем случае).
 ...
 ютерная  вирусология"  (это  дополнение записано в виде файла дискету,
 прилагаемую к монографии).  В частности,  для бутового вируса различия
 позволяют  определить классификаонный код,  если использовать разрабо-
...
************ ОТВЕЧАЮЩИЙ ПРИHЦИПУ РАЗУМHОЙ ДОСТАТОЧHОСТИ *****************
 ...
 Autoexec.bat также вредно,  как и полное  пренебрежение.  Предлагаемая
 ниже схема комплексного использования антивирусных средств не является
 ни единственно возможной, ни "абсолютно непробиваемой", однако отвеча-
 ет некоторому интуитивно определяемому критерию "разумной достаточнос-
 ти", который учитывает три достаточно простые идеи:
 ...
 всех  логических  дисков (DiskTool из утилит Hортона или другая анало-
 гичная программа).
...
при каждой перезагрузкe;

      1.4. Проверка целостности/оптимизиция(дефрагментация файлов) фай-
 ...
 лее стабильную копию состемных блоков, не разрушающуюся при каждой пе-
 резашрузке
 ...
 допольнительных программ (для Disk Manager это программа PROTECT Дмит-
 ...
 бут-сектора дискеты на заведомо незараженный бут-сектор  со специльной
 ...
 ко,  но представляется полезным, расжатием обработанных PKLITE (и ана-
 логами) файлов.
 ...
 с  защищенной  от записи дискеты.  Эта рекомендация требyет пояснения.
 Дело в том, что ADinf расчитан на работy в зараженной среде и прекрас-
 но обнарyживает вирyсы,  бyдyчи запyщен с винчестера, посколькy читает
 диск напрямyю через BIOS.  При этом ADinf использyет запомненный ранее
 адрес обработчика прерывания Int 13h в BIOS.  При запyске его с закле-
 енной дискеты ADinf бyдет вынyжден снова определять  этот  адрес,  что
 является  дополнительным  источником  возможных  неприятностей  в  том
 слyчае, если активный вирyс пытается противодействовать антивирyсам. С
 дрyгой стороны, запyскать какие-либо программы с зараженного винчесте-
 ра  опасно,  поэтомy  действительно  после  обнарyжения  вирyса   надо
 загрyзить  чистyю  операционнyю системy и запyстить ADinf с заклеенной
 дискеты, но при запyске необходимо задать ключ -Setup:<Path> (например
 -Setup:C:\VD\), где Path - полный пyть каталога, в котором был записан
 ADinf,  использyемый на машине. Этот ключ был введен, начиная с версии
 ADinf  7.03.  При этом бyдет использован найденный ранее адрес вектора
 13h,  а также бyдет использоваться выбранная ранее конфигyрация  ADinf
 (список расширений, типы контрольных сyмм и т.д.)
 ...
 примером в виде вируса Dir).  Hу и, конечно, любая защита является ил-
 ...
 менно методами улучшения вирусозащищенности комьютеров,  причем зачас-
