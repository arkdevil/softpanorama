...

{
	for (;flist != NULL; flist = flist->next) {
		if (!flist->added) {
			if (v_flag) {
				update_total_bytes(flist->dta.ff_fsize);
				update_num_files();
			}
			if ((Match_criteria) & (NAMES_MATCH))
				add_to_hash_table(flist, hashpjw(flist->dta.ff_name));
			else if ((Match_criteria) & (SIZES_MATCH))
				add_to_hash_table(flist, flist->dta.ff_fsize % HASH_TAB_SIZE);
			else if ((Match_criteria) & (TIMES_MATCH))
				add_to_hash_table(flist,
								  ((unsigned long) (flist->dta.ff_fdate << 16) +
								   (unsigned long) (flist->dta.ff_ftime)) %
								  HASH_TAB_SIZE);
			flist->added = 1;
		}
	}
...

{
	...
	for (cur = name_duped->next;
		...
		if ((!cur->f_name->printed) &&
			...
			if (n_found == 0) {
				if (v_flag)
					n_bytes += name_duped->f_name->dta.ff_fsize;
				menu[n_found++] = name_duped->f_name;
			}
			if (v_flag)
				n_bytes += cur->f_name->dta.ff_fsize;
	...
	for (cur = name_duped->next; cur != NULL; cur = cur->next)
		if ((!cur->f_name->printed) &&
			...
			if (!name_duped->f_name->printed) {
				if (v_flag) {
					update_num_which_dupd();
					update_num_dups(1U, name_duped->f_name->dta.ff_fsize);
				}
				print_match_header(name_duped->f_name);
				print_next_match(name_duped->f_name, -1);
			}
			if (v_flag)
				update_num_dups(1U, cur->f_name->dta.ff_fsize);
