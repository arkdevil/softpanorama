...
uses SysUtils, Forms, FDualLst, Consts, LibConst, ExtConst;
...
begin
  ...
  FLabel1Caption := GetExtStr(SDualListSrcCaption);
  FLabel2Caption := GetExtStr(SDualListDestCaption);
  OkBtnCaption := LoadStr(SOKButton);
  CancelBtnCaption := LoadStr(SCancelButton);
  HelpBtnCaption := LoadStr(SHelpButton);
end;
...
begin
  Result := CompareStr(Label1Caption, GetExtStr(SDualListSrcCaption)) <> 0;
end;

function TDualListDialog.IsLabel2Custom: Boolean;
begin
  Result := CompareStr(Label2Caption, GetExtStr(SDualListDestCaption)) <> 0;
end;

function TDualListDialog.IsOkBtnCustom: Boolean;
begin
  Result := CompareStr(OkBtnCaption, LoadStr(SOKButton)) <> 0;
end;

function TDualListDialog.IsCancelBtnCustom: Boolean;
begin
  Result := CompareStr(CancelBtnCaption, LoadStr(SCancelButton)) <> 0;
end;

function TDualListDialog.IsHelpBtnCustom: Boolean;
begin
  Result := CompareStr(HelpBtnCaption, LoadStr(SHelpButton)) <> 0;
end;
...
begin
  ...
  try
    with Form do begin
      ...
      if IsOkBtnCustom then OkBtn.Caption := OkBtnCaption;
      if IsCancelBtnCustom then CancelBtn.Caption := CancelBtnCaption;
      if IsHelpBtnCustom then HelpBtn.Caption := HelpBtnCaption;
...
procedure Register;
begin
  RegisterComponents(LoadStr(srDialogs), [TDualListDialog]);
end;
