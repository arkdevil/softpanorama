...
met_stack	equ	offset stack0+0b9h	;смещение к стеку
com_line	equ	offset stack0+0c0h	;смещение буфера клавиатуры
com_line1	equ	offset stack0+142h	;смещение доп.буфера клавиатуры
path0		equ	offset stack0+1d3h	;смещение пути к левой панели
path1		equ	offset stack0+21ch	;смещение пути к правой панели
path_sp		equ	offset stack0+265h	;смещение к sp.com
array_drive	equ	offset stack0+2b0h      ;смещение к массиву дисков
one_begin_wind	equ	offset stack0+2c8h	;смещение к началу экрана
...
;начальная настройка системы
                ...
                mov     sp,met_stack
		mov	dx,offset phrase_ver
		call	phrase1
		mov	cl,4
		mov	bx,one_begin_wind	;смещение к началу экрана
		shr	bx,cl
		inc	bx
		mov	ah,4ah
		int	21h
		...
		add	al,"A"
		mov	si,path_sp
		mov	ds:[si],al	;заслать текущий дисковод в путь0
...
met_303:
		...
		stosb
met_304:
		mov	word ptr ds:[ path_sp_end ],di
		mov	al,"*"
		stosb
...
body:
		...
		int	21h
		add	al,"A"
		mov	di,word ptr cs:met_path[bp]
		stosb
		mov	si,path_sp + 1
		movsw
		mov	si,di
...
met_a7:
		...
		cmp	al,dl
		jnz	met_a7
		mov	byte ptr cs:[array_drive + bx],al
		inc	bx
		jmp	met_a7
@@met:
		dec	bx
		mov	byte ptr cs:act_drive[1],bl	;всего драйверов
met_0101:
		push	cs
		pop	ds
		call	clear_buf_key
...
;ядро системы
		call	command_path
		mov	byte ptr ds:keep_file,0ffh	;запрет поиска выделен.
		cmp	byte ptr ds:act_screen,0
		jz	met_0
met_088:
...
met_00:
		...
		push	cs
		pop	ds
		or	al,al		;функциональные клавиши
		jz	met_201
		cmp	al,8		;backspace
		jz	met_200
		cmp	al,9		;tab
...
met_03:
		...
		cmp	ah,24		;Ctr+"O"
		jnz	met_01
		not	byte ptr ds:act_screen
		jmp	short met_088
met_01:
		cmp	ah,49		;Ctr+"N"	;снег
		jnz	met_0150
		not	byte ptr ds:met_cga
		jmp	short met_0
met_0150:
		cmp	ah,44		;Ctr+"Z"	;halt процессора
		jnz	met_101
		not	byte ptr ds:hlt_ass
		jmp	short met_0
met_101:
		cmp	ah,18		;Ctr+"E"
		jnz	met_04
		cmp	ds:[ met_keep_line ],cl
		jz	met_0
		push	cs
		pop	es
		mov	si,com_line1
...
met_102:
		jmp	short met_0
met_04:
		cmp	byte ptr ds:act_screen,cl
		jnz	met_102
		cmp	ah,37		;Ctr+"K"	- смена сортировок
		jnz	met_05
;сменить кодировку рамки
		mov	si,offset ds:wind
		mov	di,offset ds:winda
		call	coder
		not	byte ptr ds:met_convert
		jmp	short met_0103
met_05:
		cmp	ah,47		;Ctr+"V"	;смена видов сортировок
		jnz	met_0100
		not	byte ptr ds:met_sort
met_0103:
		mov	ds:keep_file,cl
		jmp	met_0101
met_0100:
...
met_07:
		cmp	ah,38		;Ctr+"L"
		jnz	met_08
		not	byte ptr ds:met_info
		cmp	byte ptr ds:met_info,cl
		jz	met_007
		call	info
		jmp	short met_102
met_007:
		call	indic
		mov	byte ptr ds:keep_file,cl
		jmp	met_080
met_08:
		cmp	ah,19		;Ctr+"R"
		jnz	met_102
...
;функция выхода с запросом
		mov	di,offset phrase_quit
		mov	si,offset sparrow
		call	needs
		jc	met_sim4
		call	flign
		mov	ah,4ch
...
met_quit20:
		mov	al,bl
		mov	si,offset phrase_yes
                mov     dx,0823h
		call	phrase
		mov	al,bh
		mov	dl,29h
		mov	si,offset phrase_no
		call	phrase
		xchg	bl,bh
met_quit:
		call	halt
...
met_pal2:
		push	cs
		pop	ds
		mov	si,offset param_mda
		mov	di,offset segm_wind
		call	coder
met_pal6:
		pop	ax
		pop	es
...
;▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄
new_edit	proc	near
		mov	si,offset dat_edit
		mov	di,offset create_file
		call	begin_param10
met_edit1:
		call	accept
		call	halt
...
met_e02:
		...
		push	cs
		pop	ds
		cmp	word ptr ds:drive_file,offset cs:ext_edit+3
		jz	met_e05
		xchg	bx,di
		sub	bx,di
		mov	cl,bl
...
;если ошибка,то сменить атрибуты или создать файл
		mov	si,offset ds:dat_edit
		call	begin_view
;ds = cs внутри функции
		mov	si,offset ds:dat_save
		mov	di,160*24+18
		mov	cl,4
met_e2:
		movsb
		...
		loop	met_e2
		mov	al,ds:[ fon3 ]
		mov	si,offset ds:dat_line
		mov	dx,0039h
		call	phrase
		mov	si,offset ds:dat_col
		mov	dl,45h
		call	phrase
		mov	si,offset ds:dat_char
		mov	dl,30h
		call	phrase
		call	insert
		mov	dx,ds:drive_file
...
;проверить наличие памяти для загрузки
		cmp	word ptr ds:[80h+28],0
		jnz	met_e5
		mov	cx,ds:[80h+26]
		cmp	cx,ds:size_block
		jbe	met_e6
met_e5:
		mov	al,8
		jmp	short met_e4
...
;считать файл
		mov	ds:handle0,ax
		mov	word ptr ds:long_view_file,cx	;запомнить длину файла
		mov	bp,cx
		call	beg_free_m
		cwd
		mov	bx,ax
...
met_e899:
		...
		cmp	ah,1		;Esc
		jnz	met_e900
		cmp	word ptr cs:met_edit,0	;небыло редактирования
		jz	met_e999
		call	begin_param4
		call	edit_quit
		jc	met_e899
...
met_e646:
		...
		jmp	short met_e654
met_e650:
		mov	bx,bp		;условие окончания поиска
met_e653:
		mov	di,si
		inc	di
		call	mean_simbol
...
mean_simbol:
		...
		cmp	byte ptr cs:met_convert,0ffh
		jz	mean_conv
...
edit_quit:
		...
		mov	cx,0611h
                mov     dx,093fh
		mov	si,offset dat_edit + 1
		call	window
		mov	dh,07h
		mov	si,offset last_save
		call	print_name
                inc     dh
		mov	bl,al
		mov	bh,ds:[ fon4 ]
...
insert:
		...
		pop	ds
		mov	dx,0018h
		mov	si,offset ds:dat_ins
		cmp	ds:met_ins,dh		;0
		jz	met_e611
		mov	si,offset ds:dat_over
met_e611:
		mov	al,ds:[ fon3 ]
		call	phrase
		pop	ds
...
met_e690:
		push	si
		sub	ax,ax
		xchg	word ptr cs:pos_backspace,ax
		mov	di,si
		inc	si
		inc	si
		mov	cx,bp
...
;------------------------- введена Enter -------------------------
		...
		inc	bp
		inc	bp
		add	word ptr cs:pos_simbol,2
;переход на стрелка вправо
		call	move_txt
		sub	word ptr cs:begin_txt,2
		mov	cs:begin_colon,ax	;только бы <> 0
		call	out_dx_curs
		jmp	met_e550
met_e610:
;ввести символ в буфер
		cmp	byte ptr cs:met_ins,0
		jz	met_e631
		call	over
...
;отобразить символ на экране
		cmp	dl,79
		jb	met_e602
		add	word ptr cs:begin_colon,10h
		sub	dl,0fh
met_e649:
		call	move_txt
met_e634:
...
met_e520:
		mov	dx,0a0dh
		mov	bx,17h
		cmp	byte ptr cs:pos_curs[1],23	;нижняя строка
		jz	met_e525
		mov	cx,bp
		xor	al,al		;нач. условие
...
met_e530:
		mov	dx,100h
		mov	si,cs:begin_pos_wind
		cmp	byte ptr cs:pos_curs[1],1	;верхняя строка
		jz	met_e535
		cmp	word ptr cs:begin_colon,0
		jnz	met_e532
met_e537:
...
met_e14:
		...
		call	scasb_tab
		call	cga
		cmp	si,bp	;не переставлять, контроль длинной строки
		jae	met_e11
		or	al,al
		jnz	met_e221
		inc	si
...
met_e221:
		push	bx
		sub	bx,cx
		mov	cx,bx	;база	(не ставить cl,bl)
		mov	bh,80
		sub	bh,cl	;сохранить остаток cx
		xor	al,al
		rep	stosw
		mov	cl,bh	;восстановить
		pop	bx
		add	bx,7
		and	bx,0fff8h
...
;символ с курсором
		...
		cmp	si,bp
		jb	met_out
		mov	byte ptr ds:[bp],26	;последний символ файла
met_out:
		mov	al,ds:[si]
		xor	ah,ah
		cwd
...
check_pos_cursor2:
		not	byte ptr cs:pred_point	;установить 0 - перем. на след.
		call	check_pos_cursor
		not	byte ptr cs:pred_point
		ret
e558:
...
set_old_param:
		push	cs:begin_colon
		pop	cs:old_begin_colon
		mov	byte ptr cs:old_pos_curs,dl	;только колонку
		ret
;обратный поиск
reversi_search:
		or	si,si
...
move_erase:
		...
		call	movsb_cld
met_e621:
		mov	word ptr cs:pos_backspace,0
		pop	ds
		pop	si
		pop	dx
		pop	ax
...
view		proc	near
		call	this_subdir
                jnz     met_e633
		mov	si,offset dat_view
		mov	word ptr cs:name_disp,0
		call	begin_view
		mov	si,offset dat_cnv
		mov	di,160*24+82
		mov	cl,6
met_v78:
		movsb
...
met_v2:
		...
		call	file_indic
		mov	word ptr ds:long_view_file,ax;запомнить длину файла
		mov	word ptr ds:long_view_file[2],dx
		xor	al,al	;указатель на начало
		call	file_indic
		sub	si,si
		sub	bp,bp
;считать файл в буфер
		...
		jmp	met_v30
met_v5:
		mov	byte ptr cs:[begin_buf-2],0	;нач. усл. F7
		call	clear_window
		call	convert_file
met_v3:
		call	view_screen
		mov	bx,word ptr cs:size_file
		push	ds
		pop	es
met_v14:
		call	halt
...
met_v90:
		...
		push	cs
		pop	ds
		cmp	byte ptr ds:[begin_buf-2],0	;ничего не введено
		jz	met_v14
		mov	ax,ds:begin_pos_wind
		mov	di,ds:old_pos_wind
		cmp	ax,di
		ja	met_v106
		cmp	si,di
		jb	met_v106
...
met_v30:
		...
		rep	stosw
		call	close_file
		mov	word ptr ds:keep_file,0	;объедин. keep и size
		pop	ax
		jmp	met_0101
met_v94:
		not	byte ptr cs:met_convert
		jmp	short met_v69

;--------------------------------F6-------------------------------
met_v69:
		xor	al,al		;файловый манипулятор на начало файла
		call	file_indic
		not	byte ptr cs:met_convert
		mov	word ptr cs:size_file,0
		call	read_view_file
		jc	met_v30
		jmp	short met_v166
...
;---------------------to end file-------------------------------
met_v74:
		mov	dx,word ptr cs:long_view_file[2]
		mov	ax,word ptr cs:long_view_file
		mov	cx,cs:size_block
		or	dx,dx
		jnz	met_v50
		cmp	ax,cx
...
met_v50:
		div	cx
met_v58:
		cmp	bx,ax		;на экране уже отображен конец
		jz	met_v108	;yes
		inc	ax
		mov	word ptr cs:size_file,ax
		jmp	short met_v52

;-------------------------------F7-------------------------------
met_v75:
		mov	si,offset cs:dat_view
		call	accept_search
		mov	si,ds:begin_pos_wind
		jc	met_v92
met_v82:
		mov	ds:[ old_size_file ],bx
		push	ds:[ begin_colon ]
		pop	ds:[ old_begin_colon ]
		push	ds:[ begin_line ]
		pop	ds:[ old_begin_line ]
		push	si
		mov	si,offset cs:dat_view
		call	print_search
		pop	si
...
;найден
		call	clear_window
		mov	ds:[ old_pos_wind ],si
		not	byte ptr ds:met_view_color
		push	es
		pop	ds
		call	search_wind
met_v124:
...
met_v96:
		mov	si,offset dat_view
		call	no_found
		mov	ax,ds:[ old_size_file ]
		mov	word ptr ds:[ size_file ],ax
		mov	dx,ds:[ old_begin_colon ]
		mov	cx,ds:[ old_begin_line ]
		mov	bx,ds:[ handle0 ]
		mov	ax,4200h
		int	21h
		call	read_view_file
...
met_v87:
		...
		ret
met_v91:
		cmp	byte ptr ds:[di-2],0	;ничего не введено
		jz	met_v87
		call	clear_window
		clc
		ret
print_search:
		...
		call	search_window
		mov	dh,8
		mov	si,offset searching_for
		call	print_name
		mov	es,ds:[ begin_free_memory ]
		ret
no_found:
		push	cs
		pop	ds
...
view_screen:
		...
		call	clear_screen
		mov	dx,0a0dh
		cmp	word ptr cs:met_view_color,0	;выделять?
		jz	met_v4			;нет
		mov	bx,bp
...
;конец выделения
		...
		mov	bp,bx
		mov	ah,cs:[ fon0 ]
		mov	word ptr cs:met_view_color,0
		xor	bh,bh
		jmp	short met_v7
;начало выделения
met_v103:
		...
		mov	ah,cs:[ fon3 ]
		mov	word ptr cs:met_view_color,0ffh
		jmp	short met_v7
met_v104:
		mov	al," "
		mov	cx,160*24
...
read_view_file:
		...
		int	21h
		jc	met_verror
		or	ax,ax		;была ошибка при считывании
		jz	met_v6
		mov	bp,ax		;предварит. длина файла
		mov	si,ax
		push	si
		mov	di,2
		...
		mov	bp,si		;установить длину файла
		sub	di,si		;длина удаляемой части
		mov	cx,dx
		mov	dx,ax
		sub	dx,di		;уменьшить файловый манипулятор
		mov	ax,4200h
		int	21h
met_v56:
		sub	si,si
...
check_end:
		mov	al,1
		call	file_indic
		cmp	word ptr cs:long_view_file[2],dx
		jnz	met_v55
		cmp	word ptr cs:long_view_file,ax
met_v55:
		ret
...
convert_file:
		...
		call	view_window
		mov	al,ds:[ fon4 ]
		mov	si,offset dat_conv
		mov	dh,8
		call	print_name
		mov	bx,offset tabl - 128
		mov	si,bp
met_v17:
		dec	si
		mov	al,es:[si]
...
int_24		proc	far
		mov	ax,di
		add	ax,19		;преобразование кода ошибки
		mov	byte ptr cs:met_int_24,0
		call	error
		mov	al,1
		jnc	met_error51
		mov	byte ptr cs:met_int_24,al
		add	sp,6
		push	cs
		pop	ds
		call	set_dta
...
int_21		proc	near
		...
		sub	bx,bx
		mov	es,bx
		test	byte ptr es:[410h],11000000b ;дисковых устройств два ?
		jnz	met_b4		;да
		push	ax
		push	dx
...
met_acc0:
		...
		sub	bx,bx
		mov	bl,ds:[di-2]
		mov	byte ptr ds:[di+bx],bh	;отметитить конец фразы
		push	di
		mov	dx,ds:[di-3]
...
;обработка клавиши BACKSPACE
		...
		jnz	met_acc1
		cmp	dh,cl		;dh-конец пути, cl-положение курсора
		jnz	met_acc02
		jmp	acc_quit
met_acc02:
		dec	dx
...
met_acc11:
		dec	dx
		mov	dh,cl		;dx-содер. новое полож. курсора и конца
		add	di,cx
		mov	si,di
		inc	si
		sub	cx,bp
...
met_acc2:
		cmp	ah,77
		jnz	met_acc4
		cmp	dl,cl		;длина фразы и положение курсора
		ja	met_acc21
		jmp	acc_quit
met_acc21:
		mov	dh,cl
...
met_acc8:
		mov	dh,cl		;положение курсора старое
		xchg	ax,bp
		cmp	dl,al		;длина фразы и размер буфера
		jb	met_acc81
		mov	dl,al
		xchg	ax,bp
		jmp	short met_acc10
met_acc81:
		xchg	ax,bp
		or	al,al		;символ-признак только вывода
		jz	met_acc10
		inc	dx
		inc	dh		;новое
		add	di,bp
...
met_acc10:
		pop	di
		mov	ah,byte ptr ds:[di-3]	;конец пути
		cmp	dh,dl		;положение курсора и длина фразы
		jb	met_acc17
		mov	dh,dl
met_acc17:
		cmp	dh,bh		;сравн. полож.курсора с размер. экрана
		jbe	met_acc16
		mov	bl,dh
		sub	bl,bh
		add	bl,ah		;начало фразы
		jmp	short met_acc18
met_acc16:
		mov	bl,ah
met_acc18:
		mov	ds:[di-2],dx
		mov	al,dl		;сохранить длину фразы
		mov	dl,dh		;положение символа с курсором
		sub	dl,bl		;положение курсора новое на экране
		add	dl,ah
		mov	cl,bl
		call	seg_win
		mov	di,160*23
...
met_acc83:
		add	si,cx
		sub	cl,al	;длина выводимой фразы
		neg	cl
		cmp	al,bh	;если длина фразы больше bh, то равна bh
		jb	met_acc12
		mov	cl,bh
		sub	cl,ah
met_acc12:
...
key_enter	proc	near
		...
		sub	ax,ax
		mov	al,ds:[si-2]
		cmp	ds:[si-3],al	;длина строки и начало директории
		jnz	met_ent3
		cmp	byte ptr ds:act_screen,ah
		jnz	met_ent		;выход, если на экране не оболочка
		mov	di,met_path[bp]
		call	seg_dat
		mov	dx,di
		call	locat_name
		mov	bl,ds:[si-9]		;атрибут
		inc	di
;определение конца пути к файлу
met_ent0:
		inc	di
		cmp	byte ptr cs:[di-2],"\"
		jnz	met_ent01
		mov	cx,di
...
;сохранить предыдущую директорию
		jcxz	met_ent1
		cmp	byte ptr ds:[si],"."	;выход из поддиректории?
		jnz	met_ent1		;нет
		push	si
		push	di
		push	ds
...
met_ent4:
		...
		mov	word ptr [si+bp],0dh		;Enter
		mov	ds:[bx+2],si
		mov	ax,ds:[002ch]		;сегмент окружения
...
met_ent8:
		...
		rep	cmpsb
		mov	dx,si		;смещ. к имени command.com
		jcxz	met_ent9
		jmp	short met_ent8
;в коммандной строке ничего не вводилось, ввести из таблицы файлов
met_ent18:
		mov	word ptr ds:[bx+2],offset ds:no_param
		call	indic
		call	locat_name
		mov	di,com_line
		mov	al,ds:[di-2]
...
met_ent10:
		...
		jnz	met_ent12
met_ent22:
		mov	di,offset dat_com
		call	compare
		jz	met_ent21
		mov	di,offset dat_exe
		call	compare
		jz	met_ent21
		mov	di,offset dat_bat
		call	compare
		jnz	met_ent12
		call	command_path
		jmp	met_ent4
...
keep_line:
		...
		inc	cx
		rep	movsb
		mov	byte ptr ds:met_keep_line ,0ffh
		pop	cx
		pop	di
		pop	si
		ret
...
;определяем обсолютное смещение строки с курсором на экране
		...
		mov	al,cs:[ fon0 ]
		call	cga
		cmp	word ptr max_pos0[bp],0
		jz	met_cur1
		cmp	ah,82		;Insert
		jnz	met_cur01
		test	byte ptr ds:[bx+si+1],10h
		jnz	met_cur35
		not	byte ptr ds:[bx+si]
		call	select_stat
met_cur35:
		mov	ah,80		;эмулируем нажатие стрелки вниз
met_cur01:
		cmp	byte ptr ds:[bx+si],0ffh	;файл выделен?
		jnz	met_cur1		;нет
		mov	al,cs:[ fon1 ]
met_cur1:
		call	change_cursor
		cmp	byte ptr cs:met_info,0
		jnz	met_cur13
		cmp	ah,15			;Табуляция
		jnz	met_cur13
		xor	bp,2
		...
		mov	dx,num_pos0[bp]
		shr	bp,1
		mov	word ptr cs:[ act_pan ],bp
		mov	cx,bp
		mov	byte ptr cs:act_drive,cl
		shl	bp,1
		call	write_path
		call	command_path
		call	set_new_dir
...
met_cur13:
		mov	cx,high_pos0[bp]
;----------------------------------------------
		sub	si,si		;подготовка к roll_screen
		or	bp,bp
		jz	met_cur11
		add	si,80
met_cur11:
...
met_com03:
		dec	dx
		sub	ax,ax
		cmp	byte ptr ds:met_com,al
		jnz	met_com05
		mov	dh,dl
		mov	byte ptr ds:[bx-3],dl	;номер символа после пути
		mov	word ptr ds:[bx-2],dx	;всего символов введено
met_com05:
		mov	si,bx
		...
		cmp	cl,dh
		jae	met_com82
		mov	cl,dh		;положение послед. символа и конец дир.
		mov	[bx-2],cl
met_com82:
		cmp	dl,dh
		jbe	met_com5
...
met_com12:
		...
		mov	al,cl
met_com14:
		mov	ds:[bx-2],ax	;символ с курсором и последний
		jmp	short met_com6
met_com5:
;прямой сдвиг
		mov	al,dh
		...
		rep	movsb
		mov	ah,al
		sub	word ptr ds:[bx-2],ax
		cmp	byte ptr ds:[bx-1],79
		jb	met_com6
		add	byte ptr ds:[bx-1],al	;символ с курсором
;записать новый путь
met_com6:
		mov	byte ptr ds:[bx-3],dl	;номер символа после пути
		mov	si,word ptr met_path[bp]
		mov	di,bx
		mov	cl,dl
		rep	movsb
		cmp	bx,begin_buf
...
;очистить 24 строку
met_com8:
		mov	byte ptr ds:met_com,1	;метка, что проход не первый
...
met_com04:
		xor	al,al		;метка вывода только строки
		mov	di,com_line
		call	accept
		pop	bp
		pop	es
...
;▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄
makdir		proc	near
		mov	si,offset dat_mkdir
		mov	di,offset create_dir
		call	begin_param10
met_mak1:
		call	accept
		call	halt
...
met_mak10:
		...
		int	21h
		jnc	met_mak8
		mov	ds:keep_file,ch	;0
		pop	ax
		jmp	met_080
met_mak8:
		mov	si,dx
		mov	ax,offset met_de15
		push	ax
		push	ds
		jmp	met_de777
locat_name:
...
met_us11:
		...
		sub	byte ptr cs:[6eh],3	;запомнить длину фразы
...
met_us50:
		...
		inc	di
		mov	dx,di		;сохранить начальн. позиц.
		sub	bx,bx
...
;▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄
setup		proc	near
		mov	si,offset ds:dat_setup
		mov	di,offset ds:phrase_setup
		call	needs
		jc	met_set10
		mov	si,offset ds:sp_com
		call	save_sp_name
		call	create_new_file
		jc	met_set5
		mov	bx,ax
		...
		mov	dx,100h
		xor	bp,bp
		xchg	word ptr ds:met_com,bp
		call	change_palette
		mov	cx,offset ds:stack0
		sub	cx,dx
		mov	ah,40h
		int	21h
		pushf
...
;▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄
exedit          proc    near
		mov	si,offset dat_menu+1
		mov	di,offset extern_edit
		call	needs
		jnc	met_change1
		mov	di,offset ext_edit+2
		jmp	short met_change2
met_change1:
		mov	si,offset dat_edit
		mov	di,offset extern_edit+23
		call	begin_param10
met_men1:
		call	accept
		call	halt
...
met_cd10:
		...
		mov	bp,40
met_cd11:
		mov	bh,byte ptr ds:act_drive[1]	; all drives
		sub	cl,bl
		jae	met_cd16
		add	bl,bh
		add	bl,5
...
met_cd14:
		...
		xor	bx,bx
@@cycle1:
		cmp	byte ptr ds:[array_drive+bx],al
		jz	@@exit_cycle1
		inc	bx
		loop	@@cycle1
@@exit_cycle1:
...
met_cd3:
		cmp	ah,77		;стрелка вправо
		jnz	met_cd5
		cmp	bl,bh		;имя драйвера равно максимум.
...
met_cd_quit:
		...
		ret
mul_2:
		mov	bl,byte ptr ds:act_drive[1]
		shl	bl,1
met_c00:
		ret
...
copy		proc	near
		...
		or	di,di
		jnz	met_c2
		test	byte ptr ds:[si-9],10h		;директория
		jz	met_c2
		cmp	byte ptr cs:met_renmov,0ffh
		jnz	met_c00
		cmp	byte ptr ds:[si],"."		;корневая директория
		jz	met_c00
met_c2:
		push	si
		push	cs
		pop	ds
		cmp	byte ptr ds:met_renmov,0ffh
		jnz	met_c13
		mov	si,offset dat_renmov
		call	long_window
		mov	si,offset dat_move
		jmp	short met_c16
met_c13:
		mov	si,offset dat_copy
		call	long_window
met_c16:
		mov	dx,0807h
		call	phrase
		or	di,di
		jz	met_cop30
                mov     si,offset dat_delete_files
                mov     dl,0eh
		call	phrase
                push    ax
                mov     ax,sum_sel0[bp]
...
met_c18:
		...
		jmp	short met_c12
met_c20:
		mov	byte ptr ds:[si-10],0feh	;файл удален
met_c28:
		add	si,48
		loop	met_c18
		jmp	short met_c12
...
met_c10:
		...
		jmp	short met_c31
met_c39:
		mov	byte ptr ds:[si-10],0feh
met_c12:
		mov	byte ptr cs:keep_file,0
		call	memory_free
		call	locat_name
		cmp	byte ptr cs:met_renmov,0ffh
		jnz	met_c26
;определить позицию курсора на активной панели
		...
		jcxz	met_c33
met_c37:
		cmp	byte ptr ds:[si-10],0feh	;файл удален
		jnz	met_c33
met_c30:
		add	si,48
		loop	met_c37
...
;определить положение курсора, если переименовывалась директория
		test	byte ptr cs:attrib_file,10h
		jz	met_c34
		call	indic
		cmp	num_pos0[bp],0
		jz	met_c34
		dec	word ptr num_pos0[bp]
met_c34:
		call	do_cursor
		not	byte ptr cs:keep_file
met_c31:
		call	set_cursor
		ret
inst_dir2:
...
;занести информацию о рабочих дисках
		...
		add	byte ptr cs:[di],"A"
		jmp	short met_cop02
met_cop40:
		mov	bh,byte ptr cs:[di]
		or	bh,20h
		sub	bh,"a"
		mov	byte ptr cs:drive_file[1],bh
met_cop02:
...
met_cop1:
		...
		cmp	byte ptr cs:met_renmov,0ffh
		jnz	met_c27
		mov	si,offset cs:dat_renmov+1
		call	window
		mov	si,offset dat_mov
		cmp	bl,bh			;хранятся имена рабочих дисков
...
;сделать переименование
		...
		int	21h
		jc	met_cop19
		cmp	byte ptr cs:met_overwrite,0ffh
		jz	met_cop25
		call	compare_prn
		jbe	met_cop13
		mov	al,80
...
met_cop19:
		...
		call	test_max_file
		ja	no_inc_file2
		inc	word ptr max_pos0[bp] ;для определения выдел. файлов
no_inc_file2:
		xor	bp,2
		popf
met_cop13:
		jmp	met_cop20
met_c27:
		mov	si,offset dat_copy+1
		call	window
		mov	si,offset dat_cop
met_c29:
		mov	dh,8
		push	ds
		push	cs
...
;написать имя файла в рамке
		...
		call	file_indic
		mov	si,ax		;запомнить длину файла di:si
		mov	di,dx
		mov	word ptr cs:[long_view_file],ax
		mov	word ptr cs:[long_view_file + 2],dx
		xor	al,al		;указатель на начало
		call	file_indic
;считать файл в буфер
		call	seg_win
...
met_cop41:
		...
		jc	met_cop6
met_cop4:
		test	byte ptr ds:[80h+21],00000111b
		jz	met_cop44
		call	change_attrib
		jc	met_cop6
met_cop44:
...
close_file1:
		...
		int	21h
		call	close_file3
		test	byte ptr cs:attrib_file,7
		jz	met_close0
		push	ds
		push	cs
		pop	ds
...
met_close0:
		...
		ret
set_drive0:
		mov	dl,byte ptr cs:drive_file
		jmp	short met_sd0
set_drive1:
		mov	dl,byte ptr cs:drive_file[1]
met_sd0:
		mov	ah,0eh
		call	int_21
		ret
...
met_indic:
		...
		mul	cx
                or      dx,dx           ;число больше ffff,ffff
                jnz     no_indic
		xchg	di,ax
		xchg	si,ax           ;затем младший разряд si
		mul	cx
		add	di,dx           ;складываю два результата
		xchg	si,ax
		mov	ax,word ptr cs:[long_view_file]
		mov	dx,word ptr cs:[long_view_file + 2]
		...
		or	di,di		;число не очень большое?
...
compare_prn:
		...
		push	cs
		pop	es
		mov	di,offset cs:dat_prn
		mov	si,80h+30
		call	compare
		jz	met_prn
		mov	di,offset cs:dat_con
		call	compare
		clc
		jnz	met_prn
		mov	ah,1
...
test_max_file	proc	near
		...
		cmp	word ptr cs:max_pos0[bp],max_files_in_dir
		ret
...
met_mem0:
		...
		mov	cl,4
		shr	si,cl
		add	si,ds:segm_data	;первый свободный 16 байтный блок
		mov	ds:begin_free_memory,si
...
delete		proc	near
		...
		cmp	sum_sel0[bp],cx
		jnz	met_de4
		cmp	byte ptr ds:[si],"."	;текущая директория
		jz	met_de60
met_de4:
		push	si
		push	ds
...
met_de2:
		mov	byte ptr ds:[si-10],0feh	;метка удаления 0feh
		add	si,48
		mov	ax,num_pos0[bp]
		inc	ax
		cmp	ax,max_pos0[bp]
...
met_de20:
		mov	al,bl
		mov	si,offset dat_delete+1
		mov	dx,0a20h
		call	phrase
		mov	al,bh
		mov	dl,28h
		mov	si,offset dat_cansel
		call	phrase
		xchg	bl,bh
met_de23:
		call	halt
...
;если требуется оставить выделение
		cmp	byte ptr cs:keep_file,al
		jnz	met_keep3
		mov	bx,met_dir0[bp]
		mov	di,cs:met_data
		mov	cx,max_pos0[bp]
		add	bx,10
		mov	byte ptr ds:[di],0ffh  ;только бы не 0
		jcxz	met_keep003
met_keep0:
		mov	si,bx
		cmp	byte ptr ds:[si-10],0ffh
...
met_keep003:
		...
		dec	di
		mov	word ptr cs:keep_number,di
met_keep3:
		push	cs
		pop	ds
		mov	word ptr max_pos0[bp],ax
		mov	word ptr num_pos0[bp],ax
		mov	word ptr high_pos0[bp],ax
		mov	si,met_path[bp]
		mov	di,met_dir0[bp]
		mov	word ptr ds:[si+1],"\:"
		mov	word ptr ds:[si+3],002ah	;"*",0
		call	set_dta
		cmp	ds:noread,al
...
met_r39:
		...
		cmp	byte ptr ds:[si],bl	;нахожусь в корневом каталоге
		jz	met_r00
		xor	bh,bh
;поиск конца path
met_r0:
		...
		dec	word ptr max_pos0[bp]
		sub	di,48	;начальное смещение таблицы DIR умен. на 1 поз.
met_r00:
                mov     byte ptr ds:met_root,bh
		mov	word ptr ds:[si],".*"
		mov	word ptr ds:[si+2],002ah
		mov	dx,met_path[bp]
		...
		jnz	met_exit_r2
                mov     byte ptr ds:met_error,0ffh
		call	insert_dir
                jmp     short met_r53
met_r42:
                cmp     byte ptr ds:[80h +30 ],"."
                jz      met_r40
                cmp     bh,1            ;нахожусь в корне
                jz      met_r54
                call    insert_dir
...
met_rd5:
		...
		int	21h
                ...
                jmp     short met_r53
met_r52:
                cmp     byte ptr ds:met_root,1     ;нахожусь в корне
                jnz     met_rd6
                cmp     byte ptr ds:[80h +30 ],"."
                jz      met_r1
		jmp	short met_r54
...
met_r53:
                call    insert_dir
                cmp     byte ptr ds:met_error,0ffh
                jz      met_rd2
met_r1:
...
met_r54:
		cmp	di,ds:met_data
		jb	met_r100
		mov	byte ptr ds:keep_file,0ffh ;запретить поиск выделений
met_r100:
		call	save_name
		add	di,24		;пропустить место под табл. друг. диска
		call	test_max_file
...
met_rd2:
		...
		call	select_stat
		call	command_path
		cmp	byte ptr cs:noread,0
		jnz	met_rd22
		call	set_new_dir
met_rd22:
		mov	byte ptr cs:noread,0
		call	memory_free
		call	set_cursor
		pop	es
		pop	ds
...
met_r44:
		...
		cmp	al,"Z"
		ja	met_r43
		add	byte ptr ds:[si],"a"-"A"
met_r43:
		movsb
		loop	met_r44
save_netware:
		xor	al,al
		cmp	byte ptr ds:keep_file,al ;разрешен поиск выделений
		jnz	met_keep9
		push	di
		mov	di,ds:met_data
met_keep4:
...
met_keep6:
		...
		mov	cl,13
		repnz	scasb
		cmp	di,ds:keep_number
		jb	met_keep4
met_keep8:
		pop	di
met_keep9:
		mov	byte ptr es:[di-24],al
		inc	word ptr max_pos0[bp]
		or	bl,bl		;bl = 1 -найдена позиция курсора
		jnz	met_r21
...
write_line	proc	near
		...
		mov	di,si
		sub	di,high_pos0[bp]
		mov	cx,si		;используется для хранения поз. файла
		call	mul_48
...
met_r11:
		...
		pop	ds
		mov	cl,9
		mov	si,offset ds:dat_sub_dir
		or	ah,ah
		jnz	met_r10
		mov	si,offset ds:dat_up_dir
met_r10:
		movsb
		inc	di
		loop	met_r10
...
met_fr2:
		push	cs
		pop	ds
		mov	cx,bx		;первый столбец,первая строка
		mov	dx,1628h	;23 строка,40 столбец
		add	dl,bl
		mov	si,offset sparrow
		mov	ah,byte ptr ds:[ fon0 ]
		mov	al,ah	;фон сообщения = фону поля
		call	window
		mov	di,bp
		mov	ah,byte ptr ds:wind[6]
		mov	al,byte ptr ds:wind[1]
		call	one_line
		inc	di
		inc	di
		mov	ah,byte ptr ds:wind[8]
		mov	al," "
		call	one_line
		mov	di,40
		add	di,bp
		mov	ah,byte ptr ds:wind[7]
		mov	al,byte ptr ds:wind[9]
		call	one_line	;построение mini_status линии
		mov	al,byte ptr ds:[ fon1 ]
		mov	dx,0104h	;положение выводимой фразы
		add	dl,bl
		mov	si,offset dat_name
		call	phrase		;построить надпись "Name"
		add	dl,12		;передвинуть позицию вывод. фразы
		mov	si,offset dat_size
		call	phrase
		add	dl,9
		mov	si,offset dat_date
		call	phrase
		add	dl,8
		mov	si,offset dat_time
		call	phrase
		pop	bp
		pop	ds
		ret
...
window		proc	near
		...
		sub	bx,bx
		mov	bl,ch		;строка начала окна
		mov	di,bx
		call	mul_80
                shl     di,1            ;смещ. начала строки на экране
		mov	bl,cl
		shl	bl,1		;смещ. начала окна в строке
		add	di,bx		;смещ. начала окна на экране
		mov	word ptr cs:[ begin_wind ],di
		shr	bl,1
		sub	dl,bl		;длина окна
		sub	dh,ch		;высота окна
		mov	word ptr cs:[ long_wind ],dx	;размеры окна
;запомнить окно в буфере
		mov	si,di		;начало окна на экране
		mov	di,word ptr cs:[ begin_buffer_wind ]
		mov	bx,80		;длина строки экрана
		sub	bl,dl		;длина незаполненного окна
		shl	bl,1		;длина незаполненного окна в байтах
		sub	cx,cx
		mov	es,cs:segm_data
		mov	ds,cs:segm_wind
		mov	byte ptr cs:[ cga_wait ],cl
met_a0:
		mov	cl,6
		call	wait_cga
		mov	cl,dl		;длина окна
		rep	movsw
		add	si,bx
...
;построение окна
		...
		pop	ds
		call	seg_win
		mov	di,ds:begin_wind	;начало окна на экране
		mov	si,di			;запомнить начало окна
		add	si,160			;начало следующ. строки
		mov	al,byte ptr ds:wind	;символ рамки "╔"
		stosw
		mov	cl,dl		;длина окна
		sub	cl,2		;без крайних элементов
		mov	al,byte ptr ds:wind[1]	;символ "═"
		rep	stosw
		mov	al,byte ptr ds:wind[2]	;символ "╗"
		stosw
		mov	di,si		;начало следующей строки
		mov	dh,byte ptr ds:long_wind[1] ;высота окна
		sub	dh,1		;без верхних и нижних строчек
		mov	al,byte ptr ds:wind[3]	;символ "║"
met_a1:
		mov	cl,6
		call	wait_cga
		stosw
		...
		jnz	met_a1		;нет
		mov	al,byte ptr ds:wind[4]	;символ "╚"
		stosw
		mov	cl,dl		;длина окна
		sub	cl,2		;без крайних элементов
		mov	al,byte ptr ds:wind[1]	;символ "═"
		rep	stosw
		mov	al,byte ptr ds:wind[5]	;символ "╝"
		stosw
...
met_a2:
		inc	bx
		cmp	byte ptr ds:[si+bx],0	;последний байт фразы
		jnz	met_a2		;нет

		mov	cl,bl		;длина фразы
		sub	dl,bl		;начало фразы в первой строке окна
		and	dl,not 1	;сделать число четным
		mov	di,word ptr ds:[ begin_wind ]
		add	di,dx		;начало фразы на экране
		mov	byte ptr es:[di-2]," "
met_a3:
		movsb
...
mul_80		proc	near
		...
		mov	cx,di		;запомнить результат в cx
		shl	di,1		;умножить на 2
		shl	di,1		;умножить на 2
		add	di,cx		;получить в di номер строки * 80
		pop	cx
		ret
mul_80		endp
...
met_halt0:
		...
		mov	al,00001111b
		out	dx,al
		mov	byte ptr ds:[043fh],ah	;двигатели выключены
		add	word ptr ds:[046ch],72	;измен. таймер
met_halt1:
		in	al,21h
		or	al,1
		out	21h,al		;запретить прерывания таймера
		hlt			;остановить процессор
		and	al,not 1	;разрешить таймер
		out	21h,al
		mov	ah,1
		int	16h
                jz      met_halt1
...
mul_48		proc	near
		...
		mov	cx,si		;запомнить результат в cx
...
write_path	proc	near
		...
		mov	bx,bp
		shr	bl,1
		mov	byte ptr ds:act_drive,bl
		mov	bp,0ffffh
		mov	bh,"*"
met_w0:
		inc	bp
		cmp	byte ptr [si+bp],bh	;определить длину пути
		jnz	met_w0
		sub	cx,cx
		cmp	bp,3
		jz	met_w1
		...
		jbe	met_w1		;нет
		mov	cl,byte ptr ds:[si]	;запомнить имя драйвера
		sub	bp,36
		add	si,bp		;сдвинуть начало фразы к концу
		mov	bp,36
met_w1:
		mov	byte ptr [si+bp],0
		push	bp
		mov	dx,20		;середина панели
		shr	bp,1
		inc	bp
		sub	dx,bp		;начало вывода пути на экран
		sub	di,di
		mov	bl,byte ptr ds:act_drive
		or	bl,bl
		jz	met_w2
...
met_w3:
		...
		call	phrase
		pop	bp
		mov	byte ptr [si+bp],bh
		jcxz	met_w5
		mov	al,bl
		mov	si,offset dat_path
...
met_kb1:
                ...
                ja      met_kb2
                or      al," "
                cmp     byte ptr ds:[si],"0"    ;изменить фон перед цифрой
                jb      met_kb2
		mov	ah,ds:[ fon2 ]
met_kb2:
                stosw
...
step_line:
                ...
                inc     di
		rep	stosw
		mov	word ptr es:[di],bx	;вывести символ "│"
                ret
one_line	endp
...
met_srt1:
		...
		push	bp
		mov	bp,cx
		mov	byte ptr ds:[si+13],27	;метка файла с курсором
met_srt2:
		dec	bp
		jz	met_srt10
		mov	si,dx
...
phrase		proc	near
		...
		xchg	dh,dl		;поменять местами строку и столбец
                mov     al,dh           ;сохранить столбец
		xor	dh,dh
		mov	di,dx		;строка
		call	mul_80
                mov     dl,al           ;восстановить столбец
		add	di,dx
...
met_inf0:
		...
		call	phrase
		mov	dh,2
		mov	si,offset phrase_copyright
		call	phrase
		mov	dh,3
		mov	si,offset phrase_town
		call	phrase
		mov	al,ds:wind[9]
		mov	ah,al
		call	one_line
...
;сдвинуть курсор на длину имени
		mov	ah,al
		add	word ptr ds:[bx-2],ax	;символ с курсором и последний
		pop	si
...
;▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄
error		proc	near
		cmp	byte ptr cs:met_int_24,0;фатальная ошибка уже обработана
		jz	met_error83
		mov	byte ptr cs:met_int_24,0
		stc
		ret
met_error83:
		...
		push	ds:begin_wind
		push	ds:long_wind
		add	word ptr ds:begin_buffer_wind,size_wind-270h ;смест. буфер окна
		mov	si,offset ds:dat_error
		cmp	al,80		;файл существует
		jz	met_errorf0
		cmp	al,0f0h		;вставить дискету
		jnz	met_error84
met_errorf0:
		mov	si,offset ds:dat_warning
met_error84:
		push	ax
		mov	al,ds:[ fon5 ]
		mov	ah,al
...
met_error58:
		mov	al,ds:[ fon5 ]
		mov	si,offset already_exists
		call	print_name
		inc	dh
		mov	si,offset write_old
		call	print_name
                inc     dh
		mov	bl,al
		mov	bh,ds:[ fon4 ]
...
met_error73:
		cmp	al,8
		jnz	met_error64
		mov	si,offset enough
		jmp	short met_error63
met_error64:
		cmp	al,90h		;удаление самого себя
		jnz	met_error65
		mov	si,offset inself
		jmp	short met_error63
met_error65:
		cmp	al,91h		;нет sp.mnu
		jnz	met_error66
		mov	si,offset phrase_user
		jmp	short met_error63
met_error66:
		cmp	al,92h		;нет sp.hlp
		jnz	met_error67
		mov	si,offset phrase_help
		jmp	short met_error63
met_error67:
...
met_errorf1:
		cmp	al,0f1h
		jnz	met_error71
		mov	si,offset not_command
		jmp	short met_error63
met_error71:
		cmp	al,0f0h
		jnz	met_error59
		mov	si,offset insert_disk
		mov	ds:[si+26],bl
met_error63:
		mov	al,ds:[ fon5 ]
		call	print_name
...
met_error60:
		mov	al,bl
		mov	si,offset dat_retry
		mov	dx,0e20h
		call	phrase
		mov	al,bh
		mov	dl,29h
		mov	si,offset dat_abort
		call	phrase
		xchg	bl,bh
met_error61:
		call	halt
...
met_error62:
		pushf
		call	clear_window
		sub	word ptr ds:begin_buffer_wind,size_wind-270h
		popf
		pop	ds:long_wind
		pop	ds:begin_wind
		pop	ds
...
tabl		db	"░▒▓│┤╡╢╖╕╣║╗╝╜╛┐└┴┬├─┼╞╟╚╔╩╦╠═╬╧╨╤╥╙╘╒╓╫╪┘┌█▄▌▐▀"
		...
		db	"БВЗИЙКПЮЯвалмнопрстуфхцчшщъыьэюяЁёҐґЄєІіЇї·√№¤■"
scan_tabl	db	"QWERTYUIOP[]  ASDFGHJKL;'` \ZXCVBNM1234567890-="
key_tabl_edit	dw	offset	met_e585	;"7"
		dw	offset	met_e510	;"8"
		dw	offset	met_e530	;"9"
		dw	offset	met_e899	;"-"
		dw	offset	met_e540	;"4"
		dw	offset	met_e899	;"5"
		dw	offset	met_e550	;"6"
		dw	offset	met_e899	;"+"
		dw	offset	met_e580	;"1"
		dw	offset	met_e500	;"2"
		dw	offset	met_e520	;"3"

		dw	offset	met_e640	;ctr+"4"
		dw	offset	met_e650	;ctr+"6"
		dw	offset	met_e660	;ctr+"1"
		dw	offset	met_e660	;ctr+"3"
		dw	offset	met_e680	;ctr+"7"
met_func	dw	offset	help
                dw      offset  menu
		dw	offset	view
		dw	offset	edit
		dw	offset	copy
...
met_path	dw	path0		;смещение пути к левой панели
		dw	path1		;смещение пути к правой панели
