...
// #define COLTEXT  (LIGHTGRAY+16*BLACK)
// #define COLEND   0x10
// #define COLBLOCK 0x69
#define ROWS 25
#define COLS 80
...
#define INFORM "EDITOR,Copyright(C)1993,PROZA,Nizamov"
...
#define SUBMENU WRITETEXT(BUFSIZE+1,"НЕТ ПАМЯТИ F2-С записью   F9-Без записи F5-Концы строк:     F6-Копия в .bak:   ")
...
#define WRITENUMBER2(n1) \
 c1=col+1;i=0;j=n1; \
 do symbols[i++]='0'+c1%10;while((c1/=10)>0); \
 while(i) SYM(j++)=symbols[--i]; \
 while(SYM(j)) SYM(j++)=0
...
#define READFILE \
 ...
 C:if(key==0x0D) { \
     key=fgetc(rf); \
     if(feof(rf)) {WRITESYMBOL(0x0D);break;} \
     filesize++; \
     if(key==0x0A) \
 ...
 i+=COLS,index1=index1->down,c3++) { \
   ...
   while(j<i) { \
     if(sign_p&&(c2=c1-index2->size)>=0)  \
       ...
       c1=0;continue;} \
     ATR(j)=COLTEXT+(sign_end&&c2++==0&&c3!=quanlines)*COLEND \
     +(y1<c3&&c3<y2 \
...
void WRITEFILE(f) char*f; {
  register int i;
  if(sign_mod) {
    sign_mod=0;
    if(sign_bak)
      {for(i=0;f[i]!='\0'&&f[i]!='.';i++) symbols[i]=f[i];
      symbols[i  ]='.';symbols[i+1]='b';symbols[i+2]='a';
      symbols[i+3]='k';symbols[i+4]='\0';
      rf=fopen(symbols,"wb");}
    else rf=fopen(f,"wb");
    c1=quanlines;
    ...
    fclose(rf);
    if(sign_bak) {rename(symbols,fname);rename(f,symbols);rename(fname,f);}
  }
}
...
void ALLOC() {
  if((indexfree=malloc(bricksize))==NULL)
    {PUTSUBMENU1;BIOSKEY();
    free(symbols);sign_bak=0;WRITEFILE(NAMEFILEABORT);
    abort();}
...
void main(n,nrf) int n;char*nrf[]; {
  ...
  WRITETEXT(11*COLS+20,INFORM);
  if((rf=fopen(nrf[1],"rb"))==NULL) {
    WRITETEXT(12*COLS+20,"Usage: ED <file_name>");
