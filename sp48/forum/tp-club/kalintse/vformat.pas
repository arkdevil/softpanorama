...
uses Dos,Drivers,Dialogs,Views,App,MsgBox,TpString;

procedure FormatMain(_Drv_: Char; Sd,Tr,Sc: Byte; PS_2,Sys: Boolean);
...
const text16 = ', Sector: ';
const text17 = #13^C'Format error in system area.';
...
const text56 = 'by Christoph H. Hochstatter (Germany) and Alexander V. Sessa (USSR)';
...
{$L NOSYS.OBJ}
procedure DrDos_Boot; external; { Not a valid procedure, it is binary image }
...
begin
  ...
  with regs do begin
    ...
    repeat
      ...
      intr($13,regs);
      er:=ah>1;
      if ah in [3,$80,$AA] then
      begin
        if ah = 3 then
          ExC := MessageBox(#13^C'Write protect error.', nil, mfError+mfOkButton)
        else
          ExC := MessageBox(#13^C'Drive not ready.', nil, mfError+mfOkButton);
        er := False;
        MsgDone := True;
        if ExC = cmCancel then
        begin
          ax := axs;
          Retry := 10;
          Exit;
    ...
    until not (regs.ah in [3,6,$80,$AA]);
    if er then begin
      if Retry > 0 then
      begin
        Dec(Retry);
      ...
      if ExCode = cmNo then er := False;
      if ExCode = cmYes then
      begin
        er := False;
        again := True;
      end;
...
begin
  with regs do begin
    ...
    if (FCarry and Flags) <> 0 then begin
      MessageBox(^C+text04, nil, mfOkButton);
      trk:=0;
      exit;
    end;
    if (dx and $9200)<>0 then begin
      MessageBox(^C+text05, nil, mfOkButton);
      trk:=0;
      exit;
    end;
    ...
    case buffer[1] of
      ...
      begin
        MessageBox(^C+text06, nil, mfOkButton);
        trk:=0;
        exit;
      end
    end;
    ...
    if not(lw in [0..3]) then begin
      MessageBox(^C+text07, nil, mfOkButton);
      trk:=0;
      exit;
    end;
...
begin
  ...
  Info.Good := (LongInt(trk)*LongInt(sec)*LongInt(hds)*512) div 1024;
  if gpl<>0 then
    new1E^[7]:=gpl
  else
    gpl:=new1E^[7];
  ...
  with regs do begin
    ...
    for track:=0 to trk-1 do begin
      Info.ComplPerc := Round((track+1)/(trk/100));
      n:=shift mod sec;
      ...
      for head:=0 to hds-1 do begin
        PTmpApp^.GetEvent(E);
        if (E.What = evCommand) and (E.Command = cmCancel) then begin
          while PTmpApp^.EventAvail do
            PTmpApp^.GetEvent(E);
          if MessageBox(#13+^C'Cancel operation?', nil, mfConfirmation+mfYesButton+mfNoButton) = cmYes then
            begin
              SetIntVec($1E,Old1E);
              Success := False;
              Exit;
        ...
        repeat
          ...
          es:=seg(table);
          bx:=ofs(table);
          Info.OpString := 'Format '+chr(lw+$41)+': ['+
                       Long2Str((LongInt(trk)*LongInt(sec)*LongInt(hds)*512)div 1024)+
                         'K]  H:'+Long2Str(head)+
                         ', T:'+Long2Str(Track);
          Message(DeskTop, evBroadcast, cmSendMessage, nil);
          mem[$40:$41]:=0;
          int13;
          if Retry = 10 then
          begin
          ...
          Info.OpString := 'Verify '+chr(lw+$41)+': ['+
                       Long2Str((LongInt(trk)*LongInt(sec)*LongInt(hds)*512)div 1024)+
                         'K]  H:'+Long2Str(head)+
                         ', T:'+Long2Str(Track);
          Message(DeskTop, evBroadcast, cmSendMessage, nil);
        ...
        if (FCarry and flags) <> 0 then begin
          if (x<2) or (x>10000) then begin
            if not(MsgDone) then
              MessageBox(^C+text17, nil, mfError+mfOkButton);
            SetIntVec($1E,Old1E);
            Success := False;
            Exit;
          end;
          ...
          Inc(Info.Bad,(bpb.spc*512) div 1024);
          Dec(Info.Good,(bpb.spc*512) div 1024);
          Message(DeskTop, evBroadcast, cmSendMessage, nil);
        end;
      end;
    end;
...
Procedure WriteBootSect(Sys: Boolean);
var
  InF,OutF: File;
  Att: Word;
  Readed: Word;
...
begin
  ...
  begin
    ...
    if IOResult <> 0 then
    begin
      Assign(InF,'C:\IBMBIO.COM');
      {$I-} Reset(InF,1); {$I+}
      if IOResult <> 0 then
        MessageBox(#13^C'Cannot locate your C:\IO.SYS or'#13+
                   ^C'C:\IBMBIO.COM system file.'#13+
                   ^C'Sorry...', nil, mfError+mfOkButton);
    end;
    if IOResult = 0 then
    begin
      Info.OpString := 'Writing IBMBIO.COM ...';
      Message(DeskTop, evBroadcast, cmSendMessage, nil);
      GetFAttr(InF,Att);
      Assign(OutF,para[1]+':\IBMBIO.COM');
      Rewrite(OutF,1);
      repeat
        BlockRead(InF,Buffer,SizeOf(Buffer),Readed);
        BlockWrite(OutF,Buffer,Readed);
      until Readed < SizeOf(Buffer);
      Close(InF);
      Close(OutF);
      SetFAttr(OutF,Att);
    end;
    Assign(InF,'C:\MSDOS.SYS');
    {$I-} Reset(InF,1); {$I+}
    if IOResult <> 0 then
    begin
      Assign(InF,'C:\IBMDOS.COM');
      {$I-} Reset(InF,1); {$I+}
      if IOResult <> 0 then
        MessageBox(#13^C'Cannot locate your C:\MSDOS.SYS or'#13+
                   ^C'C:\IBMDOS.COM system file.'#13+
                   ^C'Sorry...', nil, mfError+mfOkButton);
    end;
    if IOResult = 0 then
    begin
      Info.OpString := 'Writing IBMDOS.COM ...';
      Message(DeskTop, evBroadcast, cmSendMessage, nil);
      GetFAttr(InF,Att);
      Assign(OutF,para[1]+':\IBMDOS.COM');
      Rewrite(OutF,1);
      repeat
        BlockRead(InF,Buffer,SizeOf(Buffer),Readed);
        BlockWrite(OutF,Buffer,Readed);
    ...
    Assign(InF,GetEnv('COMSPEC'));
    Reset(InF,1);
    GetFAttr(InF,Att);
    Assign(OutF,para[1]+':\COMMAND.COM');
    Rewrite(OutF,1);
    Info.OpString := 'Writing COMMAND.COM ...';
    Message(DeskTop, evBroadcast, cmSendMessage, nil);
    repeat
      BlockRead(InF,Buffer,SizeOf(Buffer),Readed);
      BlockWrite(OutF,Buffer,Readed);
...
begin
  ...
  if Success then
  begin
    Info.OpString := 'Writing boot sector...';
    Message(DeskTop, evBroadcast, cmSendMessage, nil);
    WriteBootSect(Sys);
    MessageBox(#13^C'Format complete.', nil, mfInformation+mfOkButton);
  end;
{  else
    MessageBox(^C'Format cancelled.', nil, mfInformation+mfOkButton);}
end;
