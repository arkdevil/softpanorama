...
{ Commands }
  ...
  stUser1: String[10] = 'User ~1~';
  stUser2: String[10] = 'User ~2~';
...
begin
  ...
  if AOptions and fdUser1Button <> 0 then
  begin
    Insert(New(PButton, Init(R, stUser1 , cmUser1, Opt)));
    Opt := bfNormal;
    Inc(R.A.Y,3); Inc(R.B.Y,3);
  end;
  if AOptions and fdUser2Button <> 0 then
  begin
    Insert(New(PButton, Init(R, stUser2 ,cmUser2, Opt)));
    Opt := bfNormal;
    Inc(R.A.Y,3); Inc(R.B.Y,3);
  end;
...
begin
  ...
  begin
    FSplit(WildCard, Path, TName, TExt);
    if Name = '' then S := S + TName;
    if Ext = '' then
    begin
      NoWildChars(TExt);
      S := S + TExt;
...
begin
  ...
  if Event.What = evCommand then
    case Event.Command of
      cmFileOpen, cmFileReplace, cmFileClear, cmUser1, cmUser2:
        begin
