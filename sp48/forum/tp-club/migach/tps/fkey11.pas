...
BEGIN
     SetHeapMess ( 'Модуль Fkey11, процедура инициализации обьекта' );
     up := NIL;
     NEW ( point_proc );
     NEW ( label_proc );
     NEW ( param );
...
BEGIN
     SetHeapMess ( 'Модуль Fkey11, процедура сохранения параметров в стеке' );
     IF ( up <> NIL ) THEN
        up^.push_clear
     ELSE
         BEGIN
...
BEGIN
     SetHeapMess ( 'Модуль Fkey11, процедура установки параметра' );
     IF ( size = 0 ) THEN
        EXIT;
     IF ( up <> NIL ) THEN
