/*
 ...
 Имя модуля: EDFOR (EDitor of FORms)
 Автор: Вадим Низамов
...
#define KUp    0x4800
#define PgUp   0x4900
#define Shift1_Ins    0x5201
#define Shift1_Del    0x5301
#define Shift1_End    0x4F01
#define Shift1_KDown  0x5001
#define Shift1_PgDn   0x5101
#define Shift1_KLeft  0x4B01
#define Shift1_KRight 0x4D01
#define Shift1_Home   0x4701
#define Shift1_KUp    0x4801
#define Shift1_PgUp   0x4901
#define Shift1_Tab    0x0F01
...
#define SHIFT_ONN ((*address&BITSHIFT)>0)
...
#define WRITENUMBER2(n1) \
 c1=col+1L;i=0;j=n1; \
 do inform[i++]='0'+c1%10;while((c1/=10L)>0L); \
...
#define READFILE \
 ...
 for(;;) { \
   ...
   filesize++; \
 C:if(k1==0x0D) { \
     if((k1=fgetc(rf))==EOF) {WRITESYMBOL(0x0D,FREE);break;} \
     filesize++; \
     if(k1==0x0A) \
   ...
   WRITESYMBOL((unsigned char)k1,FREE); \
 } \
 index2->attr[index2->size]=FREE; \
 index1->down=NULL
...
#define READFILEATTR \
 if((rfa=fopen(namefile,"rb"))!=NULL) { \
   ...
   for(;;) { \
     if((k1=fgetc(rfa))==EOF) break; \
     key1=k1>>2;key=k1&0x03; \
     if(key1==0) \
       {index2->attr[i]=key; \
...
#define WRITEBUF \
 ...
 i+=COLS,index1=index1->down,c3++) { \
   ...
   while(j<i) \
	 ...
	 key=(index2->attr[index2->size]>=FORM)?COLFORM:COLFREE; \
	 while(j<i) \
	   {SYM(j)=0; \
	   ATT(j++)=(sign_p \
	   ||y1==c3&&y2==c3&&x1<=c4&&c4<x2 \
	   ||y1==c3&&y2!=c3&&x1<=c4 \
	   ||y1!=c3&&y2==c3&&c4<x2)?COLBLOCK:key; \
	   c4++;} \
	 break; \
       } \
       ...
       ||y1==c3&&y2!=c3&&x1<=c4 \
       ||y1!=c3&&y2==c3&&c4<x2)? \
       COLBLOCK:((index2->attr[(int)c1]>=FORM)?COLFORM:COLFREE); \
       c1++;c4++;} \
   if(c3==quanlines) \
     {while(j<BUFSIZE-COLS) {ATT(j)=COLFREE;SYM(j++)=0;} \
     break;} \
 }
...
char
  ...
  *fileforedit,
  ...
  inform[]="EDITOR OF FORMS 1.1,Copyright(C)1993,PROZA,Nizamov";
unsigned char
  COLFRAME=16*LIGHTGRAY+BLUE,
  COLFREE =16*BLUE     +WHITE,
  COLFORM =16*CYAN     +BLACK,
  COLBLOCK=16*BLACK    +YELLOW,
  key,key1,key2;
struct
  {
    unsigned char sym;
...
int BIOSKEY() {
  r.h.ah=0x08;
  int86(0x21,&r,&r);
  if((key=r.h.al)!=0) return key+SHIFT_ONN*256;
  int86(0x21,&r,&r);
  return r.h.al*256+SHIFT_ONN;
}
void WRITEFILES(f,fa) char*f,*fa; {
  register int i;
  if(sign_mod&&(rf=fopen(f,"wb"))!=NULL&&(rfa=fopen(fa,"wb"))!=NULL) {
    c1=quanlines;key=k1=sign_mod=0;
...
void main(n,nrf) int n;char*nrf[]; {
  ...
  WRITETEXT(11*COLS+14,inform);
  namefile=malloc(STRINGLENGTH);
  if(n<=1) fileforedit=NAMEFILENEW; else fileforedit=nrf[1];
  if(fileforedit[0]=='/'&&fileforedit[1]=='?')
    {WRITETEXT(13*COLS+24,"Usage:EDFOR [<file_name>]");
    puttext(1,1,COLS,ROWS,buf);BIOSKEY();return;}
  if((rf=fopen(fileforedit,"rb"))==NULL)
    {fclose(fopen(fileforedit,"wb"));
    rf=fopen(fileforedit,"rb");}
  puttext(1,1,COLS,ROWS,buf);
  for(i=0;fileforedit[i]!='\0'&&fileforedit[i]!='.';i++)
    namefile[i]=fileforedit[i];
  namefile[i  ]='.';
  namefile[i+1]='e';
  namefile[i+2]='d';
  namefile[i+3]='f';
  ...
  for(;;) {
	...
	switch(BIOSKEY()) {
	case F2:WRITEFILES(fileforedit,namefile);
	case F9:return;
	}
    ...
    case F2:
      ...
      WRITEFILES(fileforedit,namefile);
      puttextrows=ROWS;
      continue;
    case F3:
    B:SUBMENUSEARCH;
    ...
    case Tab:
      ...
      SEARCHFREE(j);
      continue;
    case Shift_End:case Shift1_End:
      sign_mark=1;
    case End:
      FINDINDEX();
      ENDOFLINE();
      MOVEXWINDOW();
      continue;
    case Shift_KDown:case Shift1_KDown:
      sign_mark=1;
    case KDown:
      FINDINDEX();
      DOWN(lin1);
      ...
      continue;
    case Shift_PgDn:case Shift1_PgDn:
      sign_mark=1;
    case PgDn:
      FINDINDEX();
      ...
      continue;
    case Shift_KLeft:case Shift1_KLeft:
      sign_mark=1;
    case KLeft:
      FINDINDEX();KURSORLEFT();MOVEXWINDOW();
      continue;
    case Shift_KRight:case Shift1_KRight:
      sign_mark=1;
    case KRight:
      FINDINDEX();KURSORRIGHT();MOVEXWINDOW();
      continue;
    case Shift_Home:case Shift1_Home:
      sign_mark=1;
    case Home:
      FINDINDEX();
      if(sign_form) col=0L;
	...
	for(j=index2->size-(int)c1,c1=0L;;)
      ...
      MOVEXWINDOW();
      continue;
    case Shift_KUp:case Shift1_KUp:
      sign_mark=1;
    case KUp:
      FINDINDEX();
      UP(lin1);
      MOVEYWINDOW(0);MOVEXWINDOW();
      continue;
    case Shift_PgUp:case Shift1_PgUp:
      sign_mark=1;
    case PgUp:
      FINDINDEX();
      UP(nfirstline);
	...
	sign_p=0;SEARCHFREE(index2->size-(int)c1);}
      continue;
    case Shift_Ins:case Shift1_Ins:
	...
	MOVEXWINDOW();WMOD();}
      continue;
    case Shift_Del:case Shift1_Del:
    case Ctrl_Del:
      if(BLOCKMARK) {WMOD();DELETEBLOCK(i!=Ctrl_Del);}
      continue;
    }
    if(key||i==F5)
