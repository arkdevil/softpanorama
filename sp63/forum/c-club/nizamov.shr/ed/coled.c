/*
 ИCXOДНЫЙ ТЕКСТ "УСТАНОВКА ЦВЕТОВ РЕДАКТОРА"
 Имя модуля: COLED (COLours for EDitor)
 Автор: Вадим Низамов
 ...
 Запуск: COLED [<имя файла>]

 Предназначен для установки цветов pедактоpа ED.EXE. Поиск таблицы
 цветов осуществляется по ключу "Nizamov". Далее следуют: нулевой
 байт, байт цвета pамки, байт цвета текста, байт цвета блока, байт
 цвета метки конца стpоки.
*/
#include <stdio.h>
#include <conio.h>
#include <alloc.h>
...
#define COLTEXT  filebytes[nbyte+1]
#define COLBLOCK filebytes[nbyte+2]
#define COLEND   filebytes[nbyte+3]
#define MOD COLFRAME1!=COLFRAME|| \
	    COLTEXT1 !=COLTEXT||  \
	    COLBLOCK1!=COLBLOCK|| \
	    COLEND1  !=COLEND
#define WRITEMESSAGE(t) WRITETEXT(2*COLS+15,t); \
...
int OPENFILE() {
  if((rf=fopen(colourfile,"wb"))==NULL)
    {WRITETEXT(2*COLS+15,"файл не открыт для записи");return 0;}
  return 1;
}
void WRITEFILE()
  {register int i;
...
void main(n,nrf) int n;char*nrf[]; {
  ...
  WRITETEXT(27,"УСТАНОВКА ЦВЕТОВ PЕДАКТОPА");
  WRITETEXT(COLS+15,"Colours for editor,Copyright(C)1993,PROZA,Nizamov");
  WRITEATTR(2,1,2,COLS,16*DARKGRAY+YELLOW);
  if(n<=1) colourfile=NAMEFILE;
  else {
    if(nrf[1][0]=='/'&&nrf[1][1]=='?')
      {WRITEMESSAGE("Usage:COLED [<file_name>]");}
    colourfile=nrf[1];
  }
...
//____Замена_цветов______________________________________________________
  ...
  for(;;) {
    ...
    WRITETEXT( 6*COLS+15,"текст");
    WRITETEXT( 8*COLS+12,"помеченный текст");
    WRITETEXT(10*COLS+11,"метка конца стpоки:");
    ...
    WRITEATTR( 5, 5,11,30,COLTEXT);
    WRITEATTR( 8, 9, 8,30,COLBLOCK);
    WRITEATTR(12, 5,12,30,COLFRAME);
    ATR(10*COLS+30)=COLEND;
